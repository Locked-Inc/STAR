...
# Run build
echo -e "${GREEN}Running: make BOARDS=STAR-Z2 BOARDDIR=$BOARD_DIR REBUILD_PYNQ_ROOTFS=1 REBUILD_PYNQ_SDIST=1${NC}"
echo ""

if make BOARDS=STAR-Z2 BOARDDIR="$BOARD_DIR" REBUILD_PYNQ_ROOTFS=1 REBUILD_PYNQ_SDIST=1; then
    # Build succeeded
    END_TIME=$(date +%s)
