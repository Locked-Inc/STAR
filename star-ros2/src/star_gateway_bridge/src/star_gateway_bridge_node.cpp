  ...
  reconnect_attempts_(0)
{
  ...
  RCLCPP_INFO(this->get_logger(),
    ...
    watchdog_timeout_sec_, teleop_timeout_ms_, grpc_deadline_ms_, wheel_base_);
...
void StarGatewayBridgeNode::initialize_ros_interfaces()
{
  ...
  // Publishers
  teleop_cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(
    "/teleop/cmd_vel", 10);
...
bool StarGatewayBridgeNode::reconnect_grpc_client()
{
  if (reconnect_attempts_ >= k_max_reconnect_attempts) {
    RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 30000,
      "Max reconnection attempts (%d) reached - giving up", k_max_reconnect_attempts);
    return false;
  }
