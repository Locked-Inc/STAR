...
TEST_F(MessageConverterTest, TwistToCommandPureRotation)
{
  ...
  double expected_velocity = 1.0 * WHEEL_BASE_M / 2.0;  // ω * L/2 = 0.075 m/s
...
TEST_F(MessageConverterTest, TwistToCommandNegativeAngular)
{
  ...
  double expected_velocity = 1.0 * WHEEL_BASE_M / 2.0;
...
TEST_F(MessageConverterTest, BatteryStateToProtoNominal)
{
  ...
  // Status
  EXPECT_EQ(
    proto_battery.status(), star::v1::BatteryState_ChargeStatus_CHARGE_STATUS_DISCHARGING);
...
{
  star::v1::PIDConfig proto_config;
  ASSERT_FALSE(converter_.pid_gains_to_proto(
    std::numeric_limits<double>::quiet_NaN(), 0.3, 0.05, proto_config));
}
...
TEST_F(MessageConverterTest, PidConfigToProtoInfinity)
{
  star::v1::PIDConfig proto_config;
  ASSERT_FALSE(converter_.pid_gains_to_proto(
    1.5, std::numeric_limits<double>::infinity(), 0.05, proto_config));
}
