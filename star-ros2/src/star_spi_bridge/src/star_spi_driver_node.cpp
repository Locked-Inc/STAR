...
StarSpiDriverNode::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  if (!spi_driver_->initialize()) {
    RCLCPP_ERROR(get_logger(), "Failed to initialize SPI driver on %s", spi_device_path_.c_str());
  ...
  obstacle_front_left_pub_ = create_publisher<sensor_msgs::msg::Range>("/star/obstacle/front_left", 10);
  obstacle_front_right_pub_ = create_publisher<sensor_msgs::msg::Range>("/star/obstacle/front_right", 10);
  obstacle_back_left_pub_ = create_publisher<sensor_msgs::msg::Range>("/star/obstacle/back_left", 10);
  obstacle_back_right_pub_ = create_publisher<sensor_msgs::msg::Range>("/star/obstacle/back_right", 10);
  obstacle_detected_pub_ = create_publisher<std_msgs::msg::Bool>("star/obstacle_detected", 10);
  estop_reason_pub_ = create_publisher<std_msgs::msg::Int32>("star/estop_reason", 10);

  // Create subscriptions
  cmd_vel_sub_ = create_subscription<geometry_msgs::msg::Twist>("cmd_vel", 10,
                                                                std::bind(&StarSpiDriverNode::cmd_vel_callback, this,
                                                                          std::placeholders::_1));
  emergency_stop_sub_ = create_subscription<std_msgs::msg::Bool>("emergency_stop", 10,
                                                                 std::bind(&StarSpiDriverNode::emergency_stop_callback,
...
StarSpiDriverNode::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  timer_ = create_wall_timer(10ms, std::bind(&StarSpiDriverNode::timer_callback, this));
...
StarSpiDriverNode::on_deactivate(const rclcpp_lifecycle::State &)
{
  ...
  std::vector<uint8_t> zero_payload(static_cast<size_t>(zero_req.ByteSizeLong()));
  if (!zero_req.SerializeToArray(zero_payload.data(), static_cast<int>(zero_payload.size()))) {
    RCLCPP_ERROR(get_logger(), "Failed to serialize zero-velocity request during deactivation");
    success = false;
  } else {
    const uint8_t zero_flags = static_cast<uint8_t>(FrameFlags::RequiresAck) |
                               static_cast<uint8_t>(FrameFlags::Priority);
    std::vector<uint8_t> zero_frame;
    SpiDriver::encode_frame(tx_seq_, FrameType::Command, zero_flags, zero_payload, zero_frame);
...
void StarSpiDriverNode::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
...
*/
void StarSpiDriverNode::emergency_stop_callback(const std_msgs::msg::Bool::SharedPtr msg)
{
  if (!msg->data) {
    return;
  }
  ...
  if (!spi_driver_) {
    RCLCPP_ERROR(get_logger(), "Emergency stop received but SPI driver is not initialized");
    return;
  }
  ...
  try {
    ...
    const uint8_t flags = static_cast<uint8_t>(FrameFlags::RequiresAck) | static_cast<uint8_t>(FrameFlags::Priority);
    ...
    if (!SpiDriver::decode_frame(rx_frame, rx_seq, rx_type, rx_flags, rx_payload)) {
      RCLCPP_ERROR(get_logger(), "Failed to decode ACK response during emergency stop");
      resume_retransmit_timer();
      return;
    }
    if (rx_type == FrameType::Nack) {
      RCLCPP_ERROR(get_logger(), "Emergency stop NACK received (seq=%u); command may not have been applied", rx_seq);
      resume_retransmit_timer();
      return;
    }
    if (rx_type != FrameType::Ack || rx_seq != tx_seq_) {
      RCLCPP_ERROR(get_logger(), "Unexpected response during emergency stop: type=0x%02X seq=%u (expected Ack seq=%u)",
...
void StarSpiDriverNode::timer_callback()
{
  try {
    ...
    if (pending_ack_ && retry_count_ < K_MAX_RETRIES && !last_tx_frame_.empty()) {
      ...
      is_retransmit = true;
      retry_count_++;
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs, "Retransmitting frame (attempt %d/%d)",
                           retry_count_, K_MAX_RETRIES);
    } else {
      if (pending_ack_ && retry_count_ >= K_MAX_RETRIES) {
        RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs,
                              "ACK timeout after %d retries; dropping frame", K_MAX_RETRIES);
        pending_ack_ = false;
        retry_count_ = 0;
      }
      ...
      *request.mutable_command() = velocity_cmd;
      ...
      if (!request.SerializeToArray(payload.data(), static_cast<int>(payload.size()))) {
        RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs,
    ...
    std::vector<uint8_t> rx_frame;
    if (!spi_driver_->transfer(tx_frame, rx_frame)) {
      RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs, "SPI Transfer Failed");
      return;
    }
    ...
    switch (rx_type) {
      ...
      case FrameType::Nack: {
        RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs, "NACK received for seq=%u", rx_seq);
      ...
      case FrameType::Response: {
        ...
        if (!telemetry.ParseFromArray(rx_payload.data(), static_cast<int>(rx_payload.size()))) {
        ...
        if (telemetry.emergency_stop()) {
          RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), kLogThrottleMs, "RX72N EMERGENCY STOP ACTIVE: reason=%s",
        ...
        const bool imu_populated = (std::fabs(imu_data.accel_z_mps2()) > kImuEpsilon ||
        ...
        if (imu_populated) {
          ...
          imu_msg.orientation_covariance = {kImuOrientationVar, 0.0, 0.0, 0.0, kImuOrientationVar, 0.0, 0.0, 0.0,
                                            kImuOrientationVar};
          ...
          imu_msg.angular_velocity_covariance = {kImuAngularVelocityVar, 0.0, 0.0, 0.0,
                                                 kImuAngularVelocityVar, 0.0, 0.0, 0.0,
          ...
          imu_msg.linear_acceleration_covariance = {kImuLinearAccelVar, 0.0, 0.0, 0.0,
                                                    kImuLinearAccelVar, 0.0, 0.0, 0.0,
                                                    kImuLinearAccelVar};
        ...
        converter_->telemetry_to_obstacle_ranges(telemetry, obs_fl, obs_fr, obs_bl, obs_br);
      ...
      default: {
        RCLCPP_DEBUG(get_logger(), "Received unhandled frame type: 0x%02X", static_cast<uint8_t>(rx_type));
        break;
      }
    }
  } catch (const std::exception & e) {
