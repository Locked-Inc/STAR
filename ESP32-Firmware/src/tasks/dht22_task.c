...
static const char* s_TAG = "DHT22_TASK"; /* Use STAR_SYSTEM_CONFIG.log_tags.dht22 */
...
{
  ESP_LOGI(s_TAG, "Initializing DHT22 on GPIO %d", STAR_SYSTEM_CONFIG.gpio.dht22_data);
  ...
  if (ret == ESP_OK) {
    ...
    ctx->last_valid_temperature = STAR_SYSTEM_CONFIG.health.default_temperature_c;
...
static esp_err_t internal_read_dht22(dht22_task_context_t* const ctx)
{
  ...
  const esp_err_t   read_result = star_bus_dht22_read(ctx->bus_manager, ctx->bus_name, &dht22_data);

  const bool data_valid = (read_result == ESP_OK && dht22_data.checksum_valid);

  const temperature_data_t temp_data =
    shared_data_create_temperature_data(dht22_data.temperature_c,
  ...
  if (data_valid) {
    ...
    const esp_err_t temp_update_ret = sensor_task_update_temperature(dht22_data.temperature_c);
    ...
    if (ctx->consecutive_failures >= STAR_SYSTEM_CONFIG.health.max_sensor_failures) {
      ESP_LOGE(s_TAG,
...
static void internal_dht22_task_loop(void* const parameter)
{
  ...
  ESP_LOGI(s_TAG,
           "DHT22 monitoring task started (interval: %dms)",
  ...
  while (ctx->task_running) {
    ...
    vTaskDelayUntil(&last_wake_time, pdMS_TO_TICKS(STAR_SYSTEM_CONFIG.tasks.dht22.interval_ms));
  }
...
esp_err_t dht22_task_start(star_bus_manager_t* const bus_manager, const char* const bus_name)
{
  ...
  s_task_context.last_valid_temperature = STAR_SYSTEM_CONFIG.health.default_temperature_c;
  ...
  const BaseType_t task_ret = xTaskCreate(internal_dht22_task_loop,
                                          STAR_SYSTEM_CONFIG.tasks.dht22.name,
                                          STAR_SYSTEM_CONFIG.tasks.dht22.stack_size,
                                          &s_task_context,
                                          STAR_SYSTEM_CONFIG.tasks.dht22.priority,
