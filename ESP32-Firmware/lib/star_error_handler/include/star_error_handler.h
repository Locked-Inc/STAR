...
/**
 * @brief Create an interface adapter for the error handler
 *
 * Fills in a star_error_interface_t structure that wraps the concrete error_handler_t
 * implementation. This allows the error handler to be used through the abstract interface.
 *
 * @param[out] iface   Pointer to interface structure to fill in
 * @param[in]  handler Pointer to the concrete error handler to wrap
 */
