...
void rx_usb_cdc_handle_bulk_in(const rx_usb_port_id_t port)
{
  ...
  if (len > k_min_transfer_size) {
    /* Write one packet.  If it exactly filled wMaxPacketSize the host
     * will keep reading until it sees a short packet or a ZLP; mark
     * the port so the next BEMP (empty-ring-buffer path below) emits
     * a terminating zero-length packet.  A <MPS packet is itself the
     * terminator, so the flag is cleared. */
    /* Rule 7: Explicitly ignore return value - errors logged by rx_usb_hw_fifo_write */
    (void)rx_usb_hw_fifo_write(pipe, data, len);
    rx_usb_tx_set_zlp_pending(port, len == k_usb_bulk_packet_size);
    return;
  }

  /* TX ring is empty.  If the previous packet was exactly MPS, emit a
   * ZLP to terminate the host read; clear the marker either way so the
   * next stream starts with a clean slate.  This runs on every board --
   * no STAR_BOARD_TOM bifurcation -- because it is standard USB bulk
   * framing practice and also resolves HOCO clock drift issues where
   * back-to-back full packets accumulate framing errors. */
  if (rx_usb_tx_zlp_pending(port)) {
    (void)rx_usb_hw_fifo_write_zlp(pipe);
    rx_usb_tx_set_zlp_pending(port, false);
  }
}
