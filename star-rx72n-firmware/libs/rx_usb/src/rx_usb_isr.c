 ...
 */
/**
 * @brief Diagnostic: number of times usb0_usbi_isr() has entered since boot.
 *
 * @details
 * Exported so the application task loop can read/emit it on a debug pin
 * and distinguish "ISR never fires" from "ISR fires but hangs / handler
 * misroutes the event".  Volatile to prevent the compiler from caching it
 * across the task/ISR boundary.
 */
