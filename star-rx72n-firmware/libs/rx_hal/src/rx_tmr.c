...
static uint32_t internal_tmr_unit_mstp_bit(rx_tmr_channel_t channel)
{
  if ((channel == k_tmr_channel_0) || (channel == k_tmr_channel_1)) {
    return (uint32_t)k_tmr_mstpcra_mstpa5;
  }
...
static uint8_t internal_tmr_compose_tcr(const rx_tmr_config_t* config)
{
  ...
  if ((config->irq_mask & (uint8_t)k_tmr_irq_cmp_match_a) != 0U) {
    tcr |= (uint8_t)k_tmr_tcr_cmiea;
  }
  if ((config->irq_mask & (uint8_t)k_tmr_irq_cmp_match_b) != 0U) {
    tcr |= (uint8_t)k_tmr_tcr_cmieb;
  }
  if ((config->irq_mask & (uint8_t)k_tmr_irq_overflow) != 0U) {
    tcr |= (uint8_t)k_tmr_tcr_ovie;
  }

  switch (config->counter_clear) {
    case k_tmr_clear_disabled:
      tcr |= (uint8_t)k_tmr_tcr_cclr_disabled;
      break;
    case k_tmr_clear_cmp_match_a:
      tcr |= (uint8_t)k_tmr_tcr_cclr_cmp_match_a;
      break;
    case k_tmr_clear_cmp_match_b:
      tcr |= (uint8_t)k_tmr_tcr_cclr_cmp_match_b;
      break;
    case k_tmr_clear_external_reset:
      tcr |= (uint8_t)k_tmr_tcr_cclr_external_sig;
      break;
    default:
      /* Caller validates */
      break;
...
static rx_err_t internal_tmr_validate_config(const rx_tmr_config_t* config)
{
  ...
  if ((uint8_t)config->output_a > (uint8_t)k_tmr_output_toggle) {
    return k_rx_err_invalid_arg;
  }
  if ((uint8_t)config->output_b > (uint8_t)k_tmr_output_toggle) {
    return k_rx_err_invalid_arg;
  }
  if ((uint8_t)config->clock_source > (uint8_t)k_tmr_clock_external_both) {
    return k_rx_err_invalid_arg;
  }
...
static void internal_tmr_program_channel(rx_tmr_channel_t channel, const rx_tmr_config_t* config)
{
  ...
  regs->tcora = (uint8_t)k_tmr_max_tcora;
  regs->tcorb = (uint8_t)k_tmr_max_tcora;
  regs->tcnt  = 0U;
}
...
rx_err_t rx_tmr_init(const rx_tmr_config_t* config)
{
  ...
  if (config->mode == k_tmr_mode_16bit_cascade) {
    ...
    s_tmr_clock_bits[(uint8_t)odd]   = (uint8_t)k_tmr_tccr_css_cascade;
...
rx_err_t rx_tmr_start(rx_tmr_channel_t channel)
{
  ...
  if (s_tmr_mode[idx] == k_tmr_mode_16bit_cascade) {
    if (channel == k_tmr_channel_0) {
      tmr1()->tccr = (uint8_t)k_tmr_tccr_css_cascade;
    } else if (channel == k_tmr_channel_2) {
      tmr3()->tccr = (uint8_t)k_tmr_tccr_css_cascade;
    }
  }
  return k_rx_ok;
}
