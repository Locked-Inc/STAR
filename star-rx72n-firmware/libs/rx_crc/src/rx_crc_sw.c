/**
 * @file rx_crc_sw.c
 * @brief Software CRC Implementations for All Supported Polynomials
 *
 * @details
 * Portable software implementations for all five CRC polynomials supported by
 * the STAR CRC library. No hardware dependencies; compiles and runs on any
 * host or embedded target.
 *
 * ## Algorithms
 *
 * | Polynomial     | Method   | Table size | Notes                            |
 * |----------------|----------|-----------|----------------------------------|
 * | CRC-32/IEEE    | Table    | 1024 B    | Reflected poly 0xEDB88320        |
 * | CRC-32C        | Table    | 1024 B    | Reflected poly 0x82F63B78        |
 * | CRC-16/IBM     | Bitwise  | 0         | Reflected poly 0xA001            |
...
/**
 * @brief CRC-32 SW constants
 */
typedef enum : uint32_t {
  k_crc32_table_size = 256U,        /**< Lookup table entry count */
  k_crc_byte_mask    = 0xFFU,       /**< Byte mask for table index */
  k_crc32_init_value = 0xFFFFFFFFU, /**< IEEE 802.3 initial value */
  k_crc32_final_xor  = 0xFFFFFFFFU, /**< IEEE 802.3 final XOR */
  k_crc_byte_shift   = 8U,          /**< Right-shift per byte iteration */
} rx_crc_sw_u32_constants_t;

/**
 * @brief CRC-16 and CRC-8 constants
 */
typedef enum : uint8_t {
  k_bits_per_byte  = 8U, /**< Bits in one byte */
  k_crc_lsb_mask   = 1U, /**< Mask for LSB */
  k_crc_shift_1bit = 1U, /**< Shift by 1 */
} rx_crc_sw_bit_constants_t;
...
/**
 * @brief CRC-CCITT (Kermit) reflected polynomial (0x1021 reflected = 0x8408)
 */
typedef enum : uint16_t {
  k_crc_ccitt_poly = 0x8408U, /**< CCITT poly, reflected (Kermit) */
  k_crc_ccitt_init = 0x0000U, /**< Initial value for Kermit variant */
} rx_crc_ccitt_constants_t;

/**
 * @brief CRC-8/Maxim (Dallas) reflected polynomial (0x31 reflected = 0x8C)
 */
typedef enum : uint8_t {
  k_crc8_maxim_poly = 0x8CU, /**< Maxim/Dallas poly, reflected */
  k_crc8_init       = 0x00U, /**< Initial value */
} rx_crc8_maxim_constants_t;
 ...
 *
 * @details Each entry is the CRC-32 for a single byte 0-255.
 * Compatible with Go's crc32.ChecksumIEEE(), Python's zlib.crc32(),
 * and Ethernet FCS.
 */
...
/**
 * @brief Precomputed CRC-32C lookup table (reflected polynomial 0x82F63B78)
 *
 * @details Each entry is the CRC-32C for a single byte 0-255.
 * Polynomial: 0x1EDC6F41 (normal form), 0x82F63B78 (reflected).
 * Used by iSCSI, SCTP, BTRFS, and modern network stacks.
 */
static const uint32_t s_crc32c_table[k_crc32_table_size] = {
...
static uint16_t internal_crc16_ibm_compute(const uint8_t* data, uint32_t len)
{
  ...
  return (uint16_t)(crc ^ k_crc16_final_xor);
}
  ...
  for (uint32_t i = 0U; i < len; i++) {
    ...
    for (uint8_t b = 0U; b < k_bits_per_byte; b++) {
      if ((crc & k_crc_lsb_mask) != 0U) {
        crc = (uint8_t)((crc >> k_crc_shift_1bit) ^ k_crc8_maxim_poly);
      } else {
        crc >>= k_crc_shift_1bit;
      }
    }
...
uint32_t internal_crc_sw_compute(rx_crc_poly_t poly, const uint8_t* data, uint32_t len)
{
  switch (poly) {
    ...
    case k_rx_crc_poly_crc16:
      return (uint32_t)internal_crc16_ibm_compute(data, len);
    case k_rx_crc_poly_crc_ccitt:
      return (uint32_t)internal_crc_ccitt_compute(data, len);
    case k_rx_crc_poly_crc32c:
      return internal_crc32c_compute(data, len);
    case k_rx_crc_poly_crc32:
    default:
      return internal_crc32_compute(data, len);
  }
}
