#!/bin/bash
# STAR RX72N Firmware - clang-tidy Static Analysis Script
#
...
# cross-compiler (rx-elf-gcc) emits RX72N-specific flags that LLVM cannot
# parse.
#
# Usage:
#   ./clang_tidy.sh              # Check mode (exit non-zero on violations)
#   ./clang_tidy.sh --fix        # Apply fixes in-place
#   ./clang_tidy.sh --check      # Explicit check mode (same as default)
#   ./clang_tidy.sh --verbose    # Verbose output
#   ./clang_tidy.sh --help       # Show this help
#
# Prerequisites:
...
# ---------------------------------------------------------------------------
# Precomputed path constants (set once at startup, reused by all functions)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
usage() {
    echo "STAR RX72N Firmware - clang-tidy Static Analysis Script"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  $0                   # Check all firmware source files"
