/* star-rx72n-firmware/src/boot/smc/lowsrc.c */
/***********************************************************************************************************************
* DISCLAIMER
...
/**
 * @file lowsrc.c
 * @brief Low-Level Stream I/O Support for CCRX/ICCRX Standard Library
 *
 * @details
 * Provides low-level file I/O functions required by the CCRX/ICCRX standard
 * library (stdio.h) to support printf(), scanf(), fopen(), etc. These functions
 * redirect standard I/O streams (stdin, stdout, stderr) to hardware peripherals
 * (typically UART for console I/O).
 *
 * **Functions Provided:**
 ...
 *
 * **STAR Project Usage:**
 * STAR firmware does **not** use standard C library I/O (printf/scanf) due to:
 ...
 * This file may be removed in future STAR releases if stdio remains disabled.
 * Currently kept for potential debugging/development use.
 *
 * @warning This file is CCRX/ICCRX-specific - GNURX uses different mechanism
 * @warning stdio functions use dynamic memory - incompatible with safety-critical code
 *
 * @see lowlvl.c Character-level I/O (charget/charput implementations)
 ...
 *
 * @par NASA Power of 10 Compliance Note:
 * stdio functions (printf/scanf) violate Rule 3 (no dynamic allocation).
 * This file is provided for debugging only - production code must not use stdio.
 */
/***********************************************************************************************************************
...
void init_iolib(void)
{
  ...
  /* Standard Input File */
  if (freopen(BSP_PRV_FPATH_STDIN, "r", stdin) == nullptr) {
  ...
  if (freopen(BSP_PRV_FPATH_STDOUT, "w", stdout) == nullptr) {
  ...
  /* Standard Error File */
  if (freopen(BSP_PRV_FPATH_STDERR, "w", stderr) == nullptr) {
