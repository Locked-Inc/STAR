...
rx_err_t rx_encoder_init(const rx_encoder_config_t* config)
{
  ...
  if (channel >= k_encoder_max_channels) {
...
rx_err_t rx_encoder_read_raw(rx_mtu_channel_t channel, uint16_t* count)
{
  ...
  if (channel >= k_encoder_max_channels || !s_encoder_initialized[channel]) {
...
rx_err_t rx_encoder_read_count(rx_mtu_channel_t channel, rx_encoder_state_t* state)
{
  ...
  s_encoder_state[channel].revolutions =
    s_encoder_state[channel].total_count / counts_per_rev;

  int32_t remainder_counts = s_encoder_state[channel].total_count % counts_per_rev;
  s_encoder_state[channel].position_deg =
    (float)(remainder_counts * 360.0f) / counts_per_rev;
...
rx_err_t rx_encoder_read_velocity(rx_mtu_channel_t channel,
                                    float            delta_time_s,
                                    float*           velocity_rps)
{
