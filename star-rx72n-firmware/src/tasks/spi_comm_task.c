 ...
 */
static rx_err_t internal_handle_velocity_command(const uint8_t* payload, size_t len)
{
 ...
 * @return RX_OK on success
 */
static rx_err_t internal_handle_estop_command(const uint8_t* payload, size_t len)
{
...
static void internal_spi_comm_loop(void)
{
  ...
  while (true) {
    ...
    if (err == RX_ERR_CRC_MISMATCH) {
      ...
      s_stats.crc_errors++;
      rx_spi_comm_send_nack(&s_spi_handle, frame.header.sequence, k_frame_flag_none);
      continue;
    }
...
rx_err_t spi_comm_task_create(motor_shared_state_t* shared_state)
{
  ...
  rx_err_t err = rspi_init_peripheral(k_spi_comm_rspi_channel, k_spi_comm_spi_mode, false);
  if (err != RX_OK) {
  ...
  if (status != TX_SUCCESS) {
    rx_log_error_int(s_tag, "Failed to create SPI comm thread, status", (int32_t)status);
