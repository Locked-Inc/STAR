 ...
 *
 * @details
 * Provides test double for USB hardware layer (USB CDC virtual serial port) to
 * enable unit testing of USB communication without actual USB peripheral or host PC.
 *
 * Enables testing of: USB CDC data transfer, USB enumeration logic (stubbed), FIFO
 * management, Error handling (stall, disconnect)
 *
 * @par Mock Capabilities: Call tracking (all HW functions), Error injection
 * (configurable return values), TX/RX FIFO simulation, No actual USB access
 *
 * @par Design: Follows same pattern as mock_error_handler.h
