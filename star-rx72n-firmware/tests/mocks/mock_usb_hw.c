...
{
  return (mock != nullptr) ? mock : &g_mock_usb_hw;
}
...
void rx_usb_cdc_handle_bulk_in(const rx_usb_port_id_t port)
{
  ...
  mock_usb_hw_record_call(m, "rx_usb_cdc_handle_bulk_in", port, 0, 0, k_rx_ok);

  /* In real implementation, this pops from TX buffer and writes to FIFO.
   * For testing, we just record that it was called with the port ID. */
}
...
void rx_usb_cdc_handle_bulk_out(const rx_usb_port_id_t port)
{
  ...
  mock_usb_hw_record_call(m, "rx_usb_cdc_handle_bulk_out", port, 0, 0, k_rx_ok);

  /* In real implementation, this reads from FIFO and pushes to RX buffer.
   * For testing, we just record that it was called with the port ID. */
}
