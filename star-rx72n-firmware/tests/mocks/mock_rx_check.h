/* star-rx72n-firmware/tests/mocks/mock_rx_check.h */
 ...
 *
 * @details
 * Provides test-friendly implementations of RX error checking macros
 * (RX_CHECK_NULL_PTR, RX_FATAL_ERROR, etc.) that work on host systems
 * without RX72N-specific assembly instructions or halt behavior.
 *
 * Key differences from real implementation:
 * - RX_FATAL_ERROR logs error but does NOT halt (allows test to continue)
 * - No assembly breakpoint instructions
 * - No watchdog or interrupt disabling
 *
 * @par Usage: Automatically included in test builds
 *
