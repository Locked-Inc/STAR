 ...
 *
 * @details
 * Provides test double for MTU (Multi-Function Timer Unit) hardware in
 * quadrature encoder phase-counting mode. Enables unit testing of motor
 * encoder drivers without RX72N hardware. Supports counter simulation,
 * overflow/underflow, and position tracking.
 *
 * Enables testing of:
 * - MTU initialization in phase-counting mode
 * - Quadrature encoder counter updates
 * - Overflow and underflow handling (16-bit counter wraparound)
 * - Position/velocity calculation algorithms
 * - Encoder error detection
 *
 * @par Test Architecture:
 * @dot
 * digraph mock_mtu_encoder {
 *   rankdir=LR;
 *   test [label="Encoder\nTests"];
 *   mock [label="Mock MTU\n(simulated counter)"];
 *   real [label="Real MTU\nPhase Counter", style=dashed];
 *   test -> mock [label="Simulate pulses"];
 ...
 * |---------|-----------|-------------|
 * | Counter simulation | Yes | Simulates 16-bit up/down counter |
 * | Overflow/underflow | Yes | Tracks counter wraparound events |
 * | Call tracking | Yes | Verifies MTU function calls |
 * | Multi-channel | Yes | Multiple MTU units supported |
 *
