...
rx_bq4050_read_temperature(rx_bus_manager_t* manager, const char* bus_name, int16_t* temperature_celsius)
{
  RX_CHECK_NULL_PTR(manager, s_tag, "manager pointer is NULL");
  RX_CHECK_NULL_PTR(bus_name, s_tag, "bus_name pointer is NULL");
  RX_CHECK_NULL_PTR(temperature_celsius, s_tag, "temperature_celsius pointer is NULL");

  uint16_t temp_0_1k;
  rx_err_t err = rx_bus_smbus_read_word_data(manager, bus_name, k_sbs_temperature, &temp_0_1k);

  if (err == k_rx_ok) {
    *temperature_celsius = internal_convert_temperature(temp_0_1k);
  }
                               ...
                               uint8_t             num_cells)
{
  ...
  /* Read temperature */
  err = rx_bq4050_read_temperature(manager, bus_name, &status->temperature_celsius);
  if (err != k_rx_ok) {
