...
typedef struct {
  ...
  int16_t  temperature_celsius;      /**< Battery pack temperature in degrees Celsius */
 ...
 * @param[in] manager Bus manager instance
 * @param[in] bus_name SMBus bus name
 * @param[out] temperature_celsius Pointer to store temperature in degrees Celsius
 *
 * @return k_rx_ok on success
 * @return k_rx_err_null_pointer if manager, bus_name, or temperature_celsius is NULL
 * @return k_rx_err_not_found if bus not found
 * @return k_rx_err_invalid_state if bus not initialized
 * @return k_rx_err_timeout if SMBus timeout
 * @return k_rx_err_crc_mismatch if PEC check fails
 */
rx_err_t
rx_bq4050_read_temperature(rx_bus_manager_t* manager, const char* bus_name, int16_t* temperature_celsius);
