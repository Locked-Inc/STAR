...
static const usb_device_descriptor_t s_device_desc = {
  ...
  .device_sub_class    = k_usb_subclass_none,
  .device_protocol     = k_usb_protocol_none,
  ...
  .config =
    {
      ...
      .num_interfaces      = k_usb_num_interfaces_cdc,
  ...
  .cdc_interface =
    {
      ...
      .interface_number   = k_usb_interface_number_control,
  ...
  .cdc_call_mgmt =
    {
      ...
      .data_interface     = k_usb_interface_number_data,
    },
  .cdc_acm =
    {
  ...
  .cdc_union =
    {
      ...
      .control_interface     = k_usb_interface_number_control,
      .subordinate_interface = k_usb_interface_number_data,
    },
  .ep3_interrupt_in =
    {
  ...
  .data_interface =
    {
      .length             = sizeof(usb_interface_descriptor_t),
      .descriptor_type    = k_usb_desc_type_interface,
      .interface_number   = k_usb_interface_number_data,
      .alternate_setting  = k_usb_alternate_setting_default,
...
static uint16_t s_control_line_state = k_usb_config_unconfigured;
...
static void internal_handle_set_configuration(uint16_t usb_value)
{
  ...
  if (config == k_usb_config_value_1) {
    ...
    /* Pipe 2: Bulk OUT (EP2) */
    err = rx_usb_hw_configure_pipe(k_usb_pipe_2,
                                   k_usb_pipe_2,
                                   false,
                                   k_usb_pipecfg_type_bulk,
                                   k_usb_bulk_packet_size);
    if (err != k_rx_ok) {
      rx_log_error(s_tag, "Failed to configure Pipe 2");
      usb0()->dcpctr |= k_usb_dcpctr_pid_stall;
      return;
    }

    /* Pipe 3: Interrupt IN (EP3) */
    err = rx_usb_hw_configure_pipe(k_usb_pipe_3,
                                   k_usb_pipe_3,
                                   true,
                                   k_usb_pipecfg_type_int,
                                   k_usb_interrupt_packet_size);
    if (err != k_rx_ok) {
      rx_log_error(s_tag, "Failed to configure Pipe 3");
      usb0()->dcpctr |= k_usb_dcpctr_pid_stall;
      return;
    }
...
static void internal_handle_class_request(uint8_t usb_request, uint16_t usb_value)
{
  switch (usb_request) {
    ...
    case k_cdc_set_control_line_state:
      internal_handle_set_control_line_state(usb_value);
      break;
...
void rx_usb_cdc_handle_setup(void)
{
  ...
  if (type == k_usb_req_type_standard) {
    ...
    internal_handle_class_request(usb_request, usb_value);
  } else {
