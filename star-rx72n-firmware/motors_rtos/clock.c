/**
 * @file clock.c
 * @brief RX72N clock init for motors_rtos: MOSC 24 MHz crystal -> PLL 240 MHz
 *        -> ICLK=120 / PCKA=120 / PCKB=60 / FCLK=60 MHz.
 *
 * @details
 * Self-contained reduced version of src/rx_clock_power_init.c, stripped of
 * the logging, asserts, simulator branches, and PPLL/USB clock paths
 * motors_rtos does not need. Configures only the main PLL.
 *
...
typedef enum : uint32_t {
  /*
   * SCKCR dividers (ICLK half-speed vs production):
