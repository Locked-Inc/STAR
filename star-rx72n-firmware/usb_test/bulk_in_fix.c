...
static const uint8_t s_dev[18] = {0x12,
                                  0x01,
                                  0x00,
                                  0x02,
                                  0xFF,
...
/* Instrumentation counters.  Each reflects an interrupt / event; we flash
 * these out via the LED pattern in the main loop so the scope on PA7 (or
 * the on-board LED visible to the eye) shows what the chip is seeing. */
static volatile uint32_t g_brdy_count      = 0; /* BRDY interrupts */
static volatile uint32_t g_bemp_count      = 0; /* BEMP interrupts */
static volatile uint32_t g_setup_count     = 0; /* SETUP received */
static volatile uint32_t g_write_count     = 0; /* cfifo_write_current calls */
static volatile uint32_t g_buf_empty_count = 0; /* INBUFM==0 observations */
...
int main(void)
{
  ...
  REG8(0x80036)  = 0x00U;
  while ((REG8(0x8003C) & 0x08U) == 0) {
    __asm__ volatile("nop");
  }
  REG16(0x80028) = 0x1710U;
  REG8(0x8002A)  = 0x00U;
  while ((REG8(0x8003C) & 0x04U) == 0) {
    __asm__ volatile("nop");
  }
  REG8(0x8101C)  = 0x01U;
  ...
  uint32_t          last_led_phase = 0;

  for (;;) {
    pc++;

    /* LED encoding (visible on PA7):
         *   not configured       -> steady ~2 Hz slow blink
         *   configured, no write -> fast blink proportional to buf_empty count
         *   configured + writing -> rapid blink -- one per write_count tick */
    uint32_t led_phase = g_configured ? (g_write_count * 2U + g_buf_empty_count) : (pc >> 18);
    if (led_phase != last_led_phase) {
      LED_TGL();
      last_led_phase = led_phase;
    }
    ...
    if (g_configured && (REG16(PIPE1CTR) & (1U << 14)) == 0U) {
      ...
      /* Select CFIFO for pipe 1 IN, 8-bit MBW, ISEL=0.
             * ISEL is a DCP-only control (for CURPIPE=0 it picks IN/OUT
             * direction); on pipe 1 it must be 0 or the hardware may not
             * arm the buffer for transmission.  Previous code had ISEL=1
             * which kept bulk IN NAKing forever. */
      REG16(CFIFOSEL) = 1U;
      /* Wait for CURPIPE latch. */
      while ((REG16(CFIFOSEL) & 0xFU) != 1U) { /* spin */
      }

      /* Payload: "BULK1 NNNN " + live interrupt counters so the host
             * can see whether the chip is actually receiving tokens. */
      static uint8_t buf[32] = "BULK1 0000 ";
      tx_counter++;
      ...
      buf[11] = (uint8_t)('B');
      buf[12] = (uint8_t)('0' + ((g_brdy_count / 100U) % 10U));
      buf[13] = (uint8_t)('0' + ((g_brdy_count / 10U) % 10U));
      buf[14] = (uint8_t)('0' + (g_brdy_count % 10U));
      buf[15] = (uint8_t)('E');
      buf[16] = (uint8_t)('0' + ((g_bemp_count / 100U) % 10U));
      buf[17] = (uint8_t)('0' + ((g_bemp_count / 10U) % 10U));
      buf[18] = (uint8_t)('0' + (g_bemp_count % 10U));
      buf[19] = (uint8_t)('\r');
      buf[20] = (uint8_t)('\n');
      cfifo_write_current(buf, 21);
      g_write_count++;
