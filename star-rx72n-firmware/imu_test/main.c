...
static void riic1_init(void)
{
  ...
  *r1(k_riic_off_icbrl) = 27U;
  ...
  /* Step 6: mode/timing. ICMR1 BCWP=1 BC=000; ACK control; line filters. */
  *r1(k_riic_off_icmr1) = 0x08U;
  *r1(k_riic_off_icmr3) = k_icmr3_ackwp;
  *r1(k_riic_off_icfer) = (uint8_t)(k_icfer_tmoe | k_icfer_male | k_icfer_nale |
...
static bool i2c_read_byte(uint8_t dev_addr, uint8_t reg_addr, uint8_t *out)
{
  ...
  *iccr2 |= k_iccr2_st;
  if (!wait_bit(icsr2, k_icsr2_start, true)) { goto stop_fail; }
  *icsr2 &= (uint8_t)~k_icsr2_start;

  if (!wait_bit(icsr2, k_icsr2_tdre, true)) { goto stop_fail; }
  *icdrt = (uint8_t)((dev_addr << 1) | 0U);
  if (!wait_bit(icsr2, k_icsr2_tend, true)) { goto stop_fail; }
  if ((*icsr2) & k_icsr2_nackf) { goto stop_fail; }
  ...
  *icmr3 |= k_icmr3_ackbt;                      /* NACK after the byte we want */
  (void)*icdrr;                                  /* dummy read clocks the byte */

  if (!wait_bit(icsr2, k_icsr2_rdrf, true)) { goto stop_fail; }
...
int main(void)
{
  ...
  sci9_debug_puts("step 5: read loop @ 5 Hz on BNO055.acc_x_lsb\n");
  uint16_t iter = 0;
  for (;;) {
    uint8_t v = 0xFFU;
    bool ok = i2c_read_byte(k_bno055_addr, k_bno055_acc_x_lsb, &v);
    sci9_debug_puts("iter=");
    sci9_debug_puthex16(iter);
    sci9_debug_puts(ok ? "  acc_x_lsb=0x" : "  FAIL v=0x");
    sci9_debug_puthex16((uint16_t)v);
    /* Always dump key RIIC1 regs to see bus state */
    sci9_debug_puts("  ICCR1=0x"); sci9_debug_puthex16((uint16_t)*r1(k_riic_off_iccr1));
    sci9_debug_puts(" ICCR2=0x"); sci9_debug_puthex16((uint16_t)*r1(k_riic_off_iccr2));
    sci9_debug_puts(" ICSR2=0x"); sci9_debug_puthex16((uint16_t)*r1(k_riic_off_icsr2));
    sci9_debug_puts(" ICBRH=0x"); sci9_debug_puthex16((uint16_t)*r1(k_riic_off_icbrh));
    sci9_debug_puts(" ICBRL=0x"); sci9_debug_puthex16((uint16_t)*r1(k_riic_off_icbrl));
    sci9_debug_puts("\n");
    busy_wait_ms(500);
    iter++;
  }
