...
void test_init_double_init(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_err_invalid_state,
                    helper_init_channel(k_tpu_channel_1));
}
...
void test_read_raw_null(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_err_null_ptr,
                    rx_tpu_encoder_read_raw(k_tpu_channel_1, NULL));
}
...
void test_read_raw_uninit(void)
{
  uint16_t count = 0;
  TEST_ASSERT_EQUAL(k_rx_err_invalid_state,
                    rx_tpu_encoder_read_raw(k_tpu_channel_1, &count));
}
...
void test_read_count_forward(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_count(k_tpu_channel_1, &state));
...
void test_read_count_forward_wrap(void)
{
  ...
  rx_encoder_state_t state;
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_count(k_tpu_channel_2, &state));
...
void test_read_count_null_state(void)
{
  TEST_ASSERT_EQUAL(k_rx_ok, helper_init_channel(k_tpu_channel_1));
  TEST_ASSERT_EQUAL(k_rx_err_null_ptr,
                    rx_tpu_encoder_read_count(k_tpu_channel_1, NULL));
}
...
void test_read_count_inverted(void)
{
  ...
  rx_encoder_state_t state;
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_count(k_tpu_channel_4, &state));
...
void test_velocity_forward(void)
{
  ...
  float velocity = 0.0f;
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_velocity(&velocity, 0.01f,
                                                 k_tpu_channel_1));

  /* Second call: 1364 counts in 1 second = 1.0 RPS */
  g_mock_tpu_regs[0].tcnt = k_test_counts_per_rev;
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_velocity(&velocity, 1.0f,
                                                 k_tpu_channel_1));
...
void test_velocity_null_ptr(void)
{
  TEST_ASSERT_EQUAL(k_rx_ok, helper_init_channel(k_tpu_channel_1));
  TEST_ASSERT_EQUAL(k_rx_err_null_ptr,
                    rx_tpu_encoder_read_velocity(NULL, 0.01f,
                                                 k_tpu_channel_1));
}
...
void test_velocity_zero_dt(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_err_invalid_arg,
                    rx_tpu_encoder_read_velocity(&velocity, 0.0f,
                                                 k_tpu_channel_1));
}
...
{
  TEST_ASSERT_EQUAL(k_rx_err_invalid_state,
                    rx_tpu_encoder_reset(k_tpu_channel_1));
}
...
void test_set_count(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_set_count(5000, k_tpu_channel_1));
...
{
  TEST_ASSERT_EQUAL(k_rx_err_invalid_state,
                    rx_tpu_encoder_set_count(100, k_tpu_channel_1));
}
...
{
  TEST_ASSERT_EQUAL(k_rx_err_invalid_state,
                    rx_tpu_encoder_deinit(k_tpu_channel_1));
}
...
void test_multi_channel_independent(void)
{
  ...
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_count(k_tpu_channel_1, &state1));
  TEST_ASSERT_EQUAL(k_rx_ok,
                    rx_tpu_encoder_read_count(k_tpu_channel_2, &state2));
