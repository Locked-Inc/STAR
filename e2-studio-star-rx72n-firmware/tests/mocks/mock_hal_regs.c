 ...
 *
 * @details
 * Implements mock register accessor functions and test helper functions
 * declared in mock_hal_regs.h. Provides RAM-based register structures
 * that mirror hardware register layout for GPIO, ADC, RIIC, and System
 * peripherals.
 *
...
__attribute__((weak)) void uart_debug_puts(const char* str)
{
  (void)str;
}

__attribute__((weak)) void uart_debug_putint(int32_t value)
{
  (void)value;
}

__attribute__((weak)) void uart_debug_puthex(uint32_t value, uint8_t digits)
{
  (void)value;
  (void)digits;
}
