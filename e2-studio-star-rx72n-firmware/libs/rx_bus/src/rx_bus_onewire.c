...
static void internal_delay_timer_init(void)
{
  ...
  RX_ASSERT(system_regs() != nullptr, "System registers unavailable");
  RX_ASSERT(cmt_ctrl() != nullptr, "CMT control registers unavailable");
  RX_ASSERT(cmt3() != nullptr, "CMT3 registers unavailable");
...
static rx_err_t internal_onewire_write_bit_callback(rx_bus_config_t* bus_config, void* user_ctx)
{
  ...
  if (state == nullptr || bus_config->handle != state) {
