                        ...
                        TX_TIMER** next_timer)
{
  ...
  TX_TRACE_IN_LINE_INSERT(TX_TRACE_TIMER_INFO_GET,
                          timer_ptr,
                          TX_POINTER_TO_ULONG_CONVERT(&ticks_left),
                          0,
                          0,
  ...
  if (internal_ptr->tx_timer_internal_list_head != TX_NULL) {
    ...
    if (active_timer_list == TX_TRUE) {
      ...
      if (internal_ptr->tx_timer_internal_list_head >= _tx_timer_current_ptr) {
        ...
        ticks_left =
          ticks_left +
          ((TX_TIMER_POINTER_DIF(_tx_timer_list_end, _tx_timer_current_ptr)) + ((ULONG)1));
      }
