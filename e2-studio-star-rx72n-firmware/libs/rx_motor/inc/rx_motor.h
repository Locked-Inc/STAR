 ...
 *
 * ### Sign-Magnitude PWM (Implemented)
 * One output drives PWM, other is held low. Direction determined by which output is active.
 *
 * **Forward (duty > 0):**
 ...
 * - Output A: Held low
 * - Output B: Held low
 * - Motor: Free-wheeling (back-EMF decay through body diodes)
 *
 * **Brake (rx_motor_stop with brake=true):**
 * - Output A: Held high
 * - Output B: Held high
 ...
 * - **Forward**: Motor driving forward (output_a active PWM)
 * - **Reverse**: Motor driving reverse (output_b active PWM)
 * - **Coasting**: Motor coasting (both outputs low, free-wheeling)
 * - **Braking**: Active braking (both outputs high, short-circuit brake)
 ...
 * | output_a | Forward PWM pin | init | Never | k_gptw_output_a/b |
 * | output_b | Reverse PWM pin | init | Never | k_gptw_output_a/b |
