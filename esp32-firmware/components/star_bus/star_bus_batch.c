/* esp32-firmware/components/star_bus/star_bus_batch.c */
...
static const char* TAG = "STAR_BATCH";
 ...
 */
static esp_err_t execute_operation(batch_context_t* ctx, uint32_t index)
{
                            ...
                            star_batch_handle_t*       handle)
{
  ...
  if (ctx == NULL) {
    ESP_LOGE(TAG, "Failed to allocate batch context");
                                   ...
                                   uint8_t             command)
{
  ...
  if (ctx->operation_count >= STAR_BATCH_MAX_OPERATIONS) {
    ESP_LOGE(TAG, "Batch full (max %d operations)", STAR_BATCH_MAX_OPERATIONS);
    return ESP_ERR_NO_MEM;
  }
...
esp_err_t star_batch_execute(star_batch_handle_t handle, star_batch_stats_t* stats)
{
  ...
  if (ctx->operation_count == 0) {
    ESP_LOGW(TAG, "Batch is empty");
    return ESP_ERR_INVALID_STATE;
  }

  if (ctx->executed) {
    ESP_LOGW(TAG, "Batch already executed");
    return ESP_ERR_INVALID_STATE;
  }
  ...
  for (uint32_t i = 0; i < ctx->operation_count; i++) {
    esp_err_t result = execute_operation(ctx, i);
    ...
    if (result == ESP_OK) {
      ...
      if (ctx->config.stop_on_error) {
        ESP_LOGW(TAG, "Stopping batch execution on error");
        break;
      }
    }
    ...
    if (ctx->config.timeout_ms > 0) {
      ...
      if (pdTICKS_TO_MS(elapsed) >= ctx->config.timeout_ms) {
        ESP_LOGE(TAG, "Batch execution timeout");
