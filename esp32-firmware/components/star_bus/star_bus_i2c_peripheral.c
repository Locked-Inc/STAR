...
static const char* TAG = "STAR_I2C_PERIPH";
 ...
 */
static esp_err_t get_i2c_port(const char* bus_name, i2c_port_t* port)
{
 ...
 */
static void peripheral_task(void* param)
{
  peripheral_state_t* state = (peripheral_state_t*)param;

  ESP_LOGI(TAG, "Peripheral task started for bus '%s'", state->bus_name);
  ...
  ESP_LOGI(TAG, "Peripheral task stopped for bus '%s'", state->bus_name);
  vTaskDelete(NULL);
}
 ...
 */
static esp_err_t start_peripheral_task(peripheral_state_t* state)
{
  ...
  BaseType_t result =
    xTaskCreate(peripheral_task, "i2c_periph", 4096, state, 5, &state->task_handle);
 ...
 */
static void stop_peripheral_task(peripheral_state_t* state)
{
  if (!state->task_running) {
    return;
  }
                                         ...
                                         const star_i2c_peripheral_config_t* config)
{
  ...
  if (config->addr_mode == STAR_I2C_ADDR_7BIT) {
    if (!STAR_I2C_ADDR_7BIT_VALID(config->address)) {
      ESP_LOGE(TAG, "Invalid 7-bit address: 0x%02x", config->address);
      return ESP_ERR_INVALID_ARG;
    }
  } else {
    if (!STAR_I2C_ADDR_10BIT_VALID(config->address)) {
      ESP_LOGE(TAG, "Invalid 10-bit address: 0x%03x", config->address);
      return ESP_ERR_INVALID_ARG;
    }
  }
  ...
  /* Get I2C port number */
  esp_err_t result = get_i2c_port(bus_name, &state->port);
  if (result != ESP_OK) {
    ESP_LOGE(TAG, "Failed to get I2C port for bus '%s'", bus_name);
    return result;
  }
  ...
  if (result != ESP_OK) {
    ESP_LOGE(TAG, "Failed to configure I2C peripheral: %s", esp_err_to_name(result));
    free(state->rx_buffer);
    free(state->tx_buffer);
    free(state->last_write_data);
    return result;
  ...
  if (result != ESP_OK) {
    ESP_LOGE(TAG, "Failed to install I2C peripheral driver: %s", esp_err_to_name(result));
    free(state->rx_buffer);
    free(state->tx_buffer);
    free(state->last_write_data);
    return result;
  ...
  result = start_peripheral_task(state);
  if (result != ESP_OK) {
    ESP_LOGE(TAG, "Failed to start peripheral task");
  ...
  ESP_LOGI(TAG, "I2C peripheral enabled on bus '%s' at address 0x%02x", bus_name, config->address);
...
esp_err_t star_bus_i2c_peripheral_disable(star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "I2C peripheral disabled on bus '%s'", bus_name);
                                               ...
                                               size_t              length)
{
  ...
  if (length > state->config.tx_buffer_size) {
    ESP_LOGE(TAG, "Response data too large: %d > %d", length, state->config.tx_buffer_size);
  ...
  if (written < 0) {
    ESP_LOGE(TAG, "Failed to write to slave tx buffer");
    return ESP_FAIL;
  }
