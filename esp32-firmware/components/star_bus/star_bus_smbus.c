...
static const char* TAG = "STAR_SMBUS";
 ...
 */
static esp_err_t validate_smbus_params(star_bus_manager_t* manager, const char* bus_name)
{
  if (manager == NULL) {
                                   ...
                                   bool                write)
{
  ...
  esp_err_t result = validate_smbus_params(manager, bus_name);
  if (result != ESP_OK) {
    return result;
  }
                                  ...
                                  uint16_t*           read_data)
{
  ...
  if (read_data == NULL) {
    ESP_LOGE(TAG, "Read data pointer is NULL");
    return ESP_ERR_INVALID_ARG;
  }
                                 ...
                                 uint8_t             length)
{
  ...
  if (length == 0 || length > STAR_SMBUS_MAX_BLOCK_SIZE) {
    ESP_LOGE(TAG, "Invalid block length: %d (must be 1-%d)", length, STAR_SMBUS_MAX_BLOCK_SIZE);
    return ESP_ERR_INVALID_ARG;
  }
                                ...
                                uint8_t*            length)
{
  ...
  if (data == NULL || length == NULL) {
    ESP_LOGE(TAG, "Data or length pointer is NULL");
    return ESP_ERR_INVALID_ARG;
  }

  if (max_length < STAR_SMBUS_MAX_BLOCK_SIZE) {
    ESP_LOGE(TAG, "Buffer too small: %d (should be >= %d)", max_length, STAR_SMBUS_MAX_BLOCK_SIZE);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (block_length == 0 || block_length > STAR_SMBUS_MAX_BLOCK_SIZE) {
    ESP_LOGE(TAG, "Invalid block length received: %d", block_length);
                                        ...
                                        uint8_t*            read_length)
{
  ...
  if (write_data == NULL || read_data == NULL || read_length == NULL) {
    ESP_LOGE(TAG, "Null pointer in block process call parameters");
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (response_length == 0 || response_length > STAR_SMBUS_MAX_BLOCK_SIZE) {
    ESP_LOGE(TAG, "Invalid response length: %d", response_length);
    return ESP_ERR_INVALID_RESPONSE;
  }
