...
static const char* TAG = "STAR_SPI_DMA";
...
static spi_host_device_t get_spi_host_from_bus(star_bus_manager_t* manager, const char* bus_name)
{
  ...
  if (config == NULL || config->type != STAR_BUS_TYPE_SPI) {
    return SPI_HOST_MAX;
  }

  return config->config.spi.host;
}
 ...
 */
static bool should_use_spi_dma(const spi_dma_state_t* state, size_t length)
{
                                     ...
                                     const star_spi_dma_config_t* config)
{
  ...
  if (host == SPI_HOST_MAX) {
    ESP_LOGE(TAG, "Bus '%s' not found or not SPI", bus_name);
  ...
  if (config->dma_channel < 1 || config->dma_channel > 2) {
    ESP_LOGE(TAG, "Invalid DMA channel: %d (must be 1-2)", config->dma_channel);
    return ESP_ERR_INVALID_ARG;
  }
                                    ...
                                    void*                   context)
{
  ...
  bool use_dma = should_use_spi_dma(state, length);

  if (!use_dma) {
    ESP_LOGD(TAG, "Using standard SPI transmit (length=%zu)", length);
    esp_err_t result = star_bus_spi_transmit(manager, bus_name, data, length, NULL);
  ...
  if (!star_spi_is_dma_capable(data, length)) {
    ESP_LOGW(TAG, "Buffer not DMA-capable, falling back to standard SPI");
    esp_err_t result = star_bus_spi_transmit(manager, bus_name, data, length, NULL);
  ...
  ESP_LOGD(TAG, "Using DMA for transmit: %zu bytes", length);
                                   ...
                                   void*                   context)
{
  ...
  if (!use_dma) {
    ESP_LOGD(TAG, "Using standard SPI receive (length=%zu)", length);
    esp_err_t result = star_bus_spi_receive(manager, bus_name, data, length, NULL);
  ...
  ESP_LOGD(TAG, "Using DMA for receive: %zu bytes", length);
                                      ...
                                      void*                   context)
{
  ...
  if (!use_dma) {
    ESP_LOGD(TAG, "Using standard SPI transceive (length=%zu)", length);
    esp_err_t result = star_bus_spi_transceive(manager, bus_name, tx_data, rx_data, length, NULL);
  ...
  if (!star_spi_is_dma_capable(tx_data, length) || !star_spi_is_dma_capable(rx_data, length)) {
    ESP_LOGW(TAG, "Buffers not DMA-capable, falling back to standard SPI");
    esp_err_t result = star_bus_spi_transceive(manager, bus_name, tx_data, rx_data, length, NULL);
  ...
  ESP_LOGD(TAG, "Using DMA for transceive: %zu bytes", length);
