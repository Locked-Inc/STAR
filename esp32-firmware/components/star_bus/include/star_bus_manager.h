...
/**
 * @brief Set a custom reset function for the bus manager's error handler.
 *
 * Allows user to provide a custom reset function that will be called when
 * the error handler exhausts all retry attempts. The reset function can
 * attempt to recover from persistent errors (e.g., by re-initializing hardware).
 *
 * @param[in] manager       Pointer to the initialized bus manager. Must not be NULL.
 * @param[in] reset_fn      Custom reset function pointer. Can be NULL to disable automatic reset.
 * @param[in] reset_context User context pointer passed to the reset function when called.
