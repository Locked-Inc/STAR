...
static const char* TAG = "star_bus_devices";
                               ...
                               bmp280_config_t*          config)
{
  ...
  if (ret != ESP_OK || chip_id != BMP280_CHIP_ID) {
    ESP_LOGE(TAG, "BMP280 init failed: chip_id=0x%02X (expected 0x%02X)", chip_id, BMP280_CHIP_ID);
  ...
  ESP_LOGI(TAG, "BMP280 initialized on bus '%s'", bus_name);
  return ESP_OK;
}
                                    ...
                                    const bmp280_config_t*    config)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "BMP280 configure ctrl_meas failed");
    return ret;
  }
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "BMP280 configure config register failed");
    return ret;
  }
...
esp_err_t star_bus_mpu6050_init(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "MPU6050 initialized on bus '%s'", bus_name);
  return ESP_OK;
}
                                     ...
                                     const mpu6050_config_t*   config)
{
  ...
  ret = star_bus_i2c_write(manager,
                           ...
                           &bytes_written);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "MPU6050 sample rate config failed");
    return ret;
  }
                                    ...
                                    mpu6050_data_t*           data)
{
  ...
  if (ret != ESP_OK || bytes_read != 14) {
    ESP_LOGE(TAG, "MPU6050 read all failed");
    return ret;
  }
                                ...
                                uint8_t                   height)
{
  ...
  for (size_t i = 0; i < sizeof(init_cmds); i++) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "SSD1306 init command %zu failed", i);
      return ret;
    }
  }

  ESP_LOGI(TAG, "SSD1306 initialized on bus '%s' (%dx%d)", bus_name, width, height);
  return ESP_OK;
}
...
esp_err_t star_bus_ssd1306_clear(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  for (int page = 0; page < 8; page++) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "SSD1306 clear page %d failed", page);
      return ret;
    }
  }
                                  ...
                                  size_t                    size)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "SSD1306 update failed");
    return ret;
  }
...
esp_err_t star_bus_ads1115_init(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "ADS1115 initialized on bus '%s'", bus_name);
  return ESP_OK;
}
                                     ...
                                     const ads1115_config_t*   config)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "ADS1115 configure failed");
    return ret;
  }
...
star_bus_pcf8574_write(const star_bus_manager_t* manager, const char* bus_name, uint8_t value)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "PCF8574 write failed");
    return ret;
  }
...
star_bus_pcf8574_read(const star_bus_manager_t* manager, const char* bus_name, uint8_t* value)
{
  ...
  if (ret != ESP_OK || bytes_read != 1) {
    ESP_LOGE(TAG, "PCF8574 read failed");
    return ret;
  }
                                  ...
                                  size_t                    len)
{
  ...
  for (size_t i = 0; i < len; i += page_size) {
    ...
    esp_err_t ret = star_bus_i2c_write(manager,
                                       ...
                                       &bytes_written);
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "AT24C256 write failed at address 0x%04X", addr);
      return ret;
    }
                                 ...
                                 size_t                    len)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "AT24C256 address write failed");
    return ret;
  }
  ...
  if (ret != ESP_OK || bytes_read != len) {
    ESP_LOGE(TAG, "AT24C256 read failed");
    return ret;
  }
...
esp_err_t star_bus_ds3231_init(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "DS3231 initialized on bus '%s'", bus_name);
  return ESP_OK;
}
                                   ...
                                   const ds3231_time_t*      time)
{
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "DS3231 set time failed");
    return ret;
  }
                                   ...
                                   ds3231_time_t*            time)
{
  ...
  if (ret != ESP_OK || bytes_read != 7) {
    ESP_LOGE(TAG, "DS3231 get time failed");
    return ret;
  }

  // Convert from BCD
  time->second = bcd_to_dec(time_data[0] & 0x7F);
  time->minute = bcd_to_dec(time_data[1] & 0x7F);
  time->hour   = bcd_to_dec(time_data[2] & 0x3F);
  time->day    = bcd_to_dec(time_data[3] & 0x07);
  time->date   = bcd_to_dec(time_data[4] & 0x3F);
                                          ...
                                          float*                    temp_celsius)
{
  ...
  if (ret != ESP_OK || bytes_read != 2) {
    ESP_LOGE(TAG, "DS3231 get temperature failed");
    return ret;
  }
