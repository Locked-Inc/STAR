...
if [ -f "./add_file_header.sh" ]; then
    ...
    # Get git root and current directory to handle path conversion
    ...
    if [ -n "$HEADER_FILES" ]; then
        for file in $HEADER_FILES; do
            ...
            # Convert git-relative path to absolute path, then to working-directory-relative path
            ...
            if [ -f "$abs_file" ]; then
                ...
                if [[ "$file" == *.sh || "$file" == *.py ]]; then
                    ...
                    if head -n 1 "$abs_file" | grep -q "^#!"; then
                        header_line=$(head -n 2 "$abs_file" | tail -n 1)
                    else
                        header_line=$(head -n 1 "$abs_file")
                    fi
                else
                    header_line=$(head -n 1 "$abs_file")
                fi
