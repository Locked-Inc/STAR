#!/usr/bin/env bash
# esp32-firmware/apply_clang_format.sh
...
if $PURGE_BACKUPS; then
    ...
    backup_dirs=""
    if [[ -d ".clang_format_backups" ]]; then
        backup_dirs=$(find .clang_format_backups -maxdepth 1 -type d -name "clang_format_backup_*" 2>/dev/null)
    fi
...
# Note: .clang_format_backups directory is already in EXCLUDE_DIRS
...
for dir in "${EXCLUDE_DIRS[@]}"; do
    ...
    if [[ "$dir" == /* ]]; then
        ...
        # Relative path with slashes - specific path exclusion
        EXCLUDE_PATTERN="$EXCLUDE_PATTERN -path ./$dir -prune -o"
        if [[ "$TARGET_DIR" != "." ]]; then
            EXCLUDE_PATTERN="$EXCLUDE_PATTERN -path $TARGET_DIR/$dir -prune -o"
        fi
    else
        # Simple directory name - exclude at any depth
