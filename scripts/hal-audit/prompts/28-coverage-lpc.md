Backfill the rx_lpc unit-test suite to 100% / 100% / 100% (line /
...
- star-rx72n-firmware/tests/mocks/mock_rx_lpc.{h,c}    (extend if needed)
- star-rx72n-firmware/libs/rx_hal/src/rx_lpc.c         (add GCOVR_EXCL_LINE
                                                        / GCOVR_EXCL_BR_LINE
                                                        only for genuinely
                                                        unreachable code,
                                                        with comment)
...
- Every test must exercise documented behavior, not implementation
  details. If a branch in rx_lpc.c is unreachable from any caller,
  ...
  why.
- Reference the RX72N HW manual chapter 11 (Low Power Consumption (sleep/standby/deep standby)) when adding tests that pin specific
...
1. Run the local coverage workflow to see exact gaps:
     bash scripts/hal-audit/devcontainer-exec.sh \
       "cd star-rx72n-firmware/tests && rm -rf build && \
        ...
        gcovr --object-directory build --filter '../libs/rx_hal/src/rx_lpc.c' \
...
2. For each uncovered line / branch, decide:
     a. Genuine missing test case -> add a Unity test in test_rx_lpc.c
...
3. Run again and confirm 100% / 100% / 100% on rx_lpc.c specifically:
     gcovr ... --filter '.*/rx_lpc.c' --fail-under-line 100 \
                                       --fail-under-function 100 \
                                       --fail-under-branch 100

4. Drop the rx_lpc excludes from .github/workflows/firmware-unit-tests.yml:
...
- ZERO magic numbers in test fixtures. Every numeric literal -- counts,
  ...
  enum with an explicit underlying type. Existing examples:
      typedef enum : uint16_t { k_test_lpc_value_first = 1234U, ... }
...
- Doxygen file-level + function-level on EVERY new test function:
  ...
  the relevant LPC register semantics), @since.
- NASA Power of 10:
    ...
    7. Every rx_doc / rx_lpc return value either checked with
...
If everything passes:
  git add -A
  git commit -m "test(rx_lpc): backfill coverage to 100/100/100 + drop CI exclude"
  git push origin bsikar/verifying-coverage-lpc
