Backfill the rx_cac unit-test suite to 100% / 100% / 100% (line /
...
- star-rx72n-firmware/tests/mocks/mock_rx_cac.{h,c}    (extend if needed)
- star-rx72n-firmware/libs/rx_hal/src/rx_cac.c         (add GCOVR_EXCL_LINE
                                                        / GCOVR_EXCL_BR_LINE
                                                        only for genuinely
                                                        unreachable code,
                                                        with comment)
...
- Every test must exercise documented behavior, not implementation
  details. If a branch in rx_cac.c is unreachable from any caller,
  ...
  why.
- Reference the RX72N HW manual chapter 11 (Clock Frequency Accuracy
  Measurement Circuit, CAC) when adding tests that pin specific
...
1. Run the local coverage workflow to see exact gaps:
     bash scripts/hal-audit/devcontainer-exec.sh \
       "cd star-rx72n-firmware/tests && rm -rf build && \
        ...
        gcovr --object-directory build --filter '../libs/rx_hal/src/rx_cac.c' \
...
2. For each uncovered line / branch, decide:
     a. Genuine missing test case -> add a Unity test in test_rx_cac.c
...
3. Run again and confirm 100% / 100% / 100% on rx_cac.c specifically:
     gcovr ... --filter '.*/rx_cac.c' --fail-under-line 100 \
                                       --fail-under-function 100 \
                                       --fail-under-branch 100

4. Drop the rx_cac excludes from .github/workflows/firmware-unit-tests.yml:
     --exclude '.*/rx_hal/inc/rx72n_cac_regs.h'
     --exclude '.*/rx_hal/inc/rx_cac.h'
     --exclude '.*/rx_hal/src/rx_cac.c'
...
- ZERO magic numbers in test fixtures. Every numeric literal -- counts,
  ...
  enum with an explicit underlying type. Existing examples:
      typedef enum : uint16_t { k_test_cac_count_first = 1234U, ... }
...
- Doxygen file-level + function-level on EVERY new test function:
  ...
  the relevant CAC register semantics), @since.
- NASA Power of 10:
    ...
    7. Every rx_doc / rx_cac return value either checked with
...
If everything passes:
  git add -A
  git commit -m "test(rx_cac): backfill coverage to 100/100/100 + drop CI exclude"
  git push origin bsikar/verifying-coverage-cac
