...
echo "[BUILD] Building star-ros2-dev Docker image..."
if [ ! -f "$PROJECT_ROOT/Dockerfile" ]; then
    echo "[FAIL] Dockerfile not found in $PROJECT_ROOT"
    exit 1
fi
...
if [ ! -x "$PROJECT_ROOT/scripts/ros2/format-ros2.sh" ]; then
    echo "[FAIL] scripts/ros2/format-ros2.sh not found or not executable"
    exit 1
fi
