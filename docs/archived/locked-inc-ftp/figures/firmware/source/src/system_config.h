 ...
 * Centralizes all hardware configuration, timing parameters, and system
 * constants Following STAR embedded best practices for maintainable
 * configuration
 *
 * This header provides type-safe configuration using structs, enums, and const
 * variables. Access all system configuration through the STAR_SYSTEM_CONFIG
 * global struct.
 */
 ...
 * IMPORTANT: This value must match STAR_SYSTEM_CONFIG.num_hcsr04_sensors in
 * system_config.c
 */
...
static const float s_star_distance_min_cm =
    0.0f; /**< Minimum valid distance in centimeters */
static const float s_star_distance_max_cm =
    400.0f; /**< Maximum valid distance in centimeters */

/* Temperature validation limits (Celsius) */
static const float s_star_temperature_min_c =
    -40.0f; /**< Minimum valid temperature in Celsius */
static const float s_star_temperature_max_c =
    80.0f; /**< Maximum valid temperature in Celsius */

/* PWM validation limits (percent) */
static const float s_star_pwm_min_percent =
    0.0f; /**< Minimum valid PWM percentage */
static const float s_star_pwm_max_percent =
    100.0f; /**< Maximum valid PWM percentage */
...
static inline bool star_validate_distance(float distance) {
  return (distance >= s_star_distance_min_cm &&
          distance <= s_star_distance_max_cm);
}
...
static inline bool star_validate_temperature(float temperature) {
  return (temperature >= s_star_temperature_min_c &&
          temperature <= s_star_temperature_max_c);
}
 ...
 */
const star_task_config_t *
star_get_task_config_by_priority(star_task_priority_t priority);
