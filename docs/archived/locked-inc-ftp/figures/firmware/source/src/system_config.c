...
star_get_task_config_by_priority(const star_task_priority_t priority) {
  switch (priority) {
...
bool star_is_valid_led_channel(const star_led_channel_t channel) {
  return channel >= k_star_led_channel_left_red &&
         channel < k_star_led_channel_max;
}
