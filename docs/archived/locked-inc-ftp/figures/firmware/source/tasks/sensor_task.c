...
static const char *s_TAG =
    "SENSORS_TASK"; /* Use STAR_SYSTEM_CONFIG.log_tags.sensors */
...
static esp_err_t internal_init_sensors(sensor_task_context_t *const ctx) {
  ...
  for (uint8_t i = 0; i < STAR_SYSTEM_CONFIG.num_hcsr04_sensors; i++) {
    esp_err_t init_ret = star_sensor_hcsr04_init(
        &ctx->sensor_handles[i], ctx->bus_manager, ctx->bus_name,
        ctx->error_iface, &sensor_configs[i]);
    if (init_ret != ESP_OK) {
      ESP_LOGE(s_TAG, "Failed to init HC-SR04 sensor %d: %s", i,
  ...
  if (ret == ESP_OK) {
    ...
    ESP_LOGI(s_TAG, "All %d HC-SR04 sensors initialized successfully",
...
static esp_err_t internal_read_single_sensor(sensor_task_context_t *const ctx,
  ...
  float distance_cm;
  esp_err_t read_result = star_sensor_hcsr04_read_distance(
      &ctx->sensor_handles[sensor_index], &distance_cm);
  ...
  if (read_result == ESP_OK) {
    const char *const sensor_name =
        (sensor_index == k_star_system_hcsr04_left) ? "Left" : "Right";
    ESP_LOGD(s_TAG, "%s sensor: %.1f cm", sensor_name, distance_cm);
  } else {
    ESP_LOGE(s_TAG, "Failed to read sensor %d: %s", sensor_index,
...
static void internal_sensor_task_loop(void *const parameter) {
  ...
  while (ctx->task_running) {
    ...
    const TickType_t target_period =
        pdMS_TO_TICKS(STAR_SYSTEM_CONFIG.tasks.sensors.interval_ms);
...
esp_err_t sensor_task_start(star_bus_manager_t *const bus_manager,
  ...
  const BaseType_t task_ret = xTaskCreate(
      internal_sensor_task_loop, STAR_SYSTEM_CONFIG.tasks.sensors.name,
      STAR_SYSTEM_CONFIG.tasks.sensors.stack_size, &s_task_context,
      STAR_SYSTEM_CONFIG.tasks.sensors.priority, &s_task_context.task_handle);
...
esp_err_t sensor_task_update_temperature(const float temperature_c) {
  ...
  for (uint8_t i = 0; i < STAR_SYSTEM_CONFIG.num_hcsr04_sensors; i++) {
    esp_err_t update_ret = star_sensor_hcsr04_set_temperature(
        &s_task_context.sensor_handles[i], temperature_c);
    if (update_ret != ESP_OK) {
      ESP_LOGE(s_TAG, "Failed to update temperature for sensor %d: %s", i,
