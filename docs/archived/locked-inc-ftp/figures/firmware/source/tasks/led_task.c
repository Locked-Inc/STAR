...
static const char *s_TAG =
    "LEDS_TASK"; /* Use STAR_SYSTEM_CONFIG.log_tags.leds */
...
static esp_err_t
internal_update_led_from_sensor_data(led_task_context_t *const ctx,
                                     const sensor_data_t *const sensor_data) {
  if (!star_validate_sensor_index(sensor_data->sensor_index)) {
  ...
  if (sensor_data->is_valid && sensor_data->read_result == ESP_OK) {
    ...
    if (ret == ESP_OK) {
      const char *const sensor_name =
          (sensor_data->sensor_index == k_star_system_hcsr04_left) ? "Left"
                                                                   : "Right";

      const rgb_color_t color =
          led_controller_distance_to_color(sensor_data->distance_cm);
      if (color.red > STAR_SYSTEM_CONFIG.pwm.off_threshold ||
        ...
        ESP_LOGD(s_TAG, "%s LED: R=%.1f%% G=%.1f%% B=%.1f%% (%.1fcm)",
                 sensor_name, color.red, color.green, color.blue,
                 sensor_data->distance_cm);
      } else {
        ESP_LOGD(s_TAG, "%s LED: OFF (%.1fcm > %.0fcm)", sensor_name,
                 sensor_data->distance_cm,
                 STAR_SYSTEM_CONFIG.distance.max_distance_cm);
      }
    }
  } else {
    ret = led_controller_turn_off(&ctx->led_controller,
                                  (led_index_t)sensor_data->sensor_index);
    if (ret == ESP_OK) {
      const char *const sensor_name =
          (sensor_data->sensor_index == k_star_system_hcsr04_left) ? "Left"
  ...
  if (ret == ESP_OK) {
    ...
    ctx->update_failures++;
    ESP_LOGE(s_TAG, "Failed to update LED %d: %s", sensor_data->sensor_index,
             esp_err_to_name(ret));
  }
...
static void internal_process_sensor_data_queue(led_task_context_t *const ctx) {
  ...
  while (processed < max_per_cycle &&
         xQueueReceive(shared_ctx->sensor_data_queue, &sensor_data, 0) ==
             pdTRUE) {
...
static void
internal_update_leds_from_latest_readings(led_task_context_t *const ctx) {
  for (uint8_t i = 0; i < STAR_SYSTEM_CONFIG.num_hcsr04_sensors; i++) {
    ...
    if (shared_data_get_sensor_reading(i, &sensor_data)) {
      const uint32_t age_ms =
          shared_data_get_uptime_ms() - sensor_data.timestamp_ms;

      if (age_ms > (STAR_SYSTEM_CONFIG.tasks.sensors.interval_ms * 5)) {
        ESP_LOGW(s_TAG, "Sensor %d data too old (%lums), turning LED off", i,
                 age_ms);
...
static void internal_led_task_loop(void *const parameter) {
  led_task_context_t *const ctx = (led_task_context_t *)parameter;

  ESP_LOGI(s_TAG, "LED display task started (interval: %dms)",
...
esp_err_t led_task_start(const pca9685_handle_t *const pca_handle) {
  if (pca_handle == NULL) {
  ...
  esp_err_t ret =
      led_controller_init(&s_task_context.led_controller, pca_handle);
  if (ret != ESP_OK) {
  ...
  const BaseType_t task_ret = xTaskCreate(
      internal_led_task_loop, STAR_SYSTEM_CONFIG.tasks.leds.name,
      STAR_SYSTEM_CONFIG.tasks.leds.stack_size, &s_task_context,
      STAR_SYSTEM_CONFIG.tasks.leds.priority, &s_task_context.task_handle);
...
esp_err_t led_task_stop(void) {
  ...
  ESP_LOGI(s_TAG,
           ...
           s_task_context.update_failures, s_task_context.successful_updates);
...
bool led_task_get_stats(uint32_t *update_failures,
                        uint32_t *successful_updates) {
  if (!s_g_context_initialized || update_failures == NULL ||
      successful_updates == NULL) {
    return false;
  }
...
esp_err_t led_task_manual_update(const uint8_t sensor_index,
  ...
  if (distance_cm < 0.0f) {
    ret = led_controller_turn_off(&s_task_context.led_controller,
                                  (led_index_t)sensor_index);
    ESP_LOGI(s_TAG, "Manually turned off LED %d", sensor_index);
  } else {
    ret = led_controller_update_distance(
        &s_task_context.led_controller, (led_index_t)sensor_index, distance_cm);
    ESP_LOGI(s_TAG, "Manually updated LED %d with distance %.1fcm",
             sensor_index, distance_cm);
  }
