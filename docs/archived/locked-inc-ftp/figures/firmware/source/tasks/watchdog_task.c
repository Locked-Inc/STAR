...
static const char *s_TAG =
    "WATCHDOG_TASK"; /* Use STAR_SYSTEM_CONFIG.log_tags.watchdog */
...
static const float s_watchdog_temp_mostly_failing_rate =
    0.8f; /**< Mostly failing rate threshold */
static const float s_watchdog_percent_multiplier =
    100.0f; /**< Convert rate to percentage */

/* Timing Constants */
static const uint32_t s_watchdog_recovery_delay_ms =
    100; /**< Recovery delay in milliseconds */
...
static watchdog_health_level_t internal_assess_sensor_health(void) {
  ...
  if (xSemaphoreTake(
          shared_ctx->health_mutex,
          pdMS_TO_TICKS(STAR_SYSTEM_CONFIG.freertos.mutex_timeout_ms)) ==
      pdTRUE) {
  ...
  if (hcsr04_reads > 0) {
    ...
    if (hcsr04_failure_rate > STAR_SYSTEM_CONFIG.health.failure_rate_critical) {
      ESP_LOGE(s_TAG, "Critical HC-SR04 failure rate: %.1f%%",
               hcsr04_failure_rate * s_watchdog_percent_multiplier);
      hcsr04_health = k_watchdog_health_critical;
    } else if (hcsr04_failure_rate >
               STAR_SYSTEM_CONFIG.health.failure_rate_degraded) {
      ESP_LOGW(s_TAG, "Elevated HC-SR04 failure rate: %.1f%%",
               hcsr04_failure_rate * s_watchdog_percent_multiplier);
      hcsr04_health = k_watchdog_health_degraded;
    } else {
      ESP_LOGI(s_TAG, "HC-SR04 health good: %.1f%% failure rate",
               hcsr04_failure_rate * s_watchdog_percent_multiplier);
    }
  } else {
  ...
  if (temp_reads > 0) {
    ...
    if (temp_failure_rate == s_watchdog_temp_complete_failure_rate) {
      ESP_LOGD(s_TAG, "DHT22 completely failed (likely not connected): %.1f%%",
      ...
      ESP_LOGW(s_TAG, "DHT22 mostly failing: %.1f%%",
               temp_failure_rate * s_watchdog_percent_multiplier);
      temp_health = k_watchdog_health_degraded;
    } else {
      ESP_LOGI(s_TAG, "DHT22 health good: %.1f%% failure rate",
               temp_failure_rate * s_watchdog_percent_multiplier);
    }
  } else {
...
static watchdog_health_level_t internal_assess_task_health(void) {
  ...
  /* Optional task failures don't degrade system health if sensor isn't
   * connected */
...
static esp_err_t
internal_take_recovery_action(const watchdog_health_level_t health_level) {
  ...
  case k_watchdog_health_fault:
    ...
    ESP_LOGE(s_TAG, "System restart in %d ms...",
             STAR_SYSTEM_CONFIG.health.restart_delay_ms);
...
static void internal_watchdog_task_loop(void *const parameter) {
  ...
  ESP_LOGI(s_TAG, "System watchdog task started (interval: %dms)",
           STAR_SYSTEM_CONFIG.tasks.watchdog.interval_ms);

  TickType_t last_wake_time = xTaskGetTickCount();

  while (ctx->task_running) {
    const watchdog_health_level_t current_health =
        internal_assess_overall_health();
    ctx->last_health_level = current_health;
    ctx->health_checks_performed++;

    if (current_health != k_watchdog_health_good) {
      ESP_LOGW(s_TAG, "Health check %lu: Level %d",
               ctx->health_checks_performed, current_health);
      internal_take_recovery_action(current_health);
    } else {
      ESP_LOGI(s_TAG, "Health check %lu: System healthy",
               ctx->health_checks_performed);
    }

    if (ctx->health_checks_performed %
            STAR_SYSTEM_CONFIG.health.health_check_log_interval ==
        0) {
      const uint32_t uptime = shared_data_get_uptime_ms();
      ESP_LOGI(s_TAG,
               "Watchdog status - uptime: %lu ms, checks: %lu, recoveries: %lu",
               uptime, ctx->health_checks_performed,
...
esp_err_t watchdog_task_start(void) {
  ...
  const BaseType_t task_ret = xTaskCreate(
      internal_watchdog_task_loop, STAR_SYSTEM_CONFIG.tasks.watchdog.name,
      STAR_SYSTEM_CONFIG.tasks.watchdog.stack_size, &s_task_context,
      STAR_SYSTEM_CONFIG.tasks.watchdog.priority, &s_task_context.task_handle);
...
esp_err_t watchdog_task_stop(void) {
  ...
  ESP_LOGI(s_TAG, "Watchdog task stopped (checks: %lu, recoveries: %lu)",
...
esp_err_t watchdog_task_register_task(const TaskHandle_t task_handle,
  ...
  ESP_LOGI(s_TAG, "Registered task for monitoring (timeout: %lums)",
