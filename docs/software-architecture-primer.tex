...
\begin{titlepage}
    ...
    \begin{tikzpicture}[scale=0.8]
        % External Internet
        \node[draw, rectangle, rounded corners, fill=blue!20, minimum width=4cm, minimum height=1cm] (internet) at (0,6) {External Internet};

        % Controller App
        \node[draw, rectangle, rounded corners, fill=green!20, minimum width=3.5cm, minimum height=1.5cm] (controller) at (-5,3) {\begin{tabular}{c}Controller App\\(PWA)\end{tabular}};

        % RPi5
        \node[draw, rectangle, rounded corners, fill=orange!20, minimum width=5cm, minimum height=2cm] (rpi) at (0,3) {\begin{tabular}{c}Raspberry Pi 5\\Spring Boot Gateway\\ROS 2 / SLAM\end{tabular}};

        % ESP32
        \node[draw, rectangle, rounded corners, fill=red!20, minimum width=3cm, minimum height=1.5cm] (esp32) at (-3,0) {\begin{tabular}{c}ESP32-S3\\Motor Control\end{tabular}};

        % LiDAR
        \node[draw, rectangle, rounded corners, fill=purple!20, minimum width=3cm, minimum height=1.5cm] (lidar) at (3,0) {\begin{tabular}{c}SICK TiM561\\LiDAR\end{tabular}};

        % Connections
        \draw[->, thick] (internet) -- (rpi) node[midway, right] {WiFi STA};
        \draw[<->, thick] (controller) -- (rpi) node[midway, above] {gRPC/GraphQL};
        \draw[<->, thick] (rpi) -- (esp32) node[midway, left] {SPI + TCP};
        \draw[<->, thick] (rpi) -- (lidar) node[midway, right] {Ethernet};
    \end{tikzpicture}
...
\begin{description}[style=nextline, leftmargin=2cm]
    \item[COLA-B] The communication protocol used by SICK LiDAR sensors (like the TiM561). A binary protocol over TCP/IP for commanding the sensor and receiving scan data.
...
\begin{enumerate}
    ...
    \item \textbf{RPi5 Gateway $\rightarrow$ LiDAR}: Over Ethernet TCP/IP (already defined)
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    \node[right=3cm of internet, font=\small, align=left] (wifi_note) {
        WiFi STA: Connect to home/office WiFi\\
        WiFi AP: Create \texttt{star-robot} network
    };

    % Controller App
    \node[box, fill=green!20, below left=2cm and 2cm of internet] (controller) {Controller App\\(PWA - any device)};
    ...
    % LiDAR
    \node[box, fill=purple!20, below right=2cm and 0cm of rpi] (lidar) {SICK TiM561\\LiDAR 270\textdegree};

    % Connections
    \draw[arrow] (internet) -- node[right, font=\small] {WiFi STA} (rpi);
    \draw[arrow, <->] (controller) -- node[above, font=\small, sloped] {gRPC/GraphQL} (rpi);
    \draw[arrow, <->] (hand) -- node[left, font=\small] {WiFi AP} (rpi);
    \draw[arrow, <->] (rpi) -- node[left, font=\small] {SPI + TCP} (esp32);
    \draw[arrow, <->] (rpi) -- node[right, font=\small] {Ethernet} (lidar);
...
\begin{tikzpicture}[
    ...
    \node[sublayer, fill=purple!20] (lidar) at (0,0.5) {SICK TiM561\\COLA-B Protocol};
    ...
    \draw[->, thick] (gateway) -- node[right, font=\small] {TCP} (lidar);
...
\begin{enumerate}
    \item \textbf{Compute Efficiency} -- Optimized for resource-constrained platforms like RPi5. Achieves 10+ Hz map updates with 270-degree LiDAR scans.

    \item \textbf{Indoor Accuracy} -- Sub-centimeter accuracy in structured indoor environments. Excellent performance with the SICK TiM561's 0.33-degree angular resolution.
...
slam_toolbox:
  ros__parameters:
    ...
    max_laser_range: 10.0     # TiM561 max range
...
\begin{tikzpicture}[
    ...
    \node[layer, fill=orange!20] (hal) at (0,1.8) {
        ...
        Esp32SpiAdapter | Esp32TcpAdapter | SickLidarAdapter | Ros2Bridge
    };
    ...
    \node[right, font=\footnotesize] at (6.8,5.4) {15 Hz (LiDAR)};
...
\begin{description}[style=nextline, leftmargin=1cm]
    ...
    \item[Layer 3 - SLAM/Mapping (ROS 2 SLAM Toolbox)] Processes LiDAR scans to build occupancy grid maps. Handles loop closure detection and map optimization. Updates at LiDAR scan rate (15 Hz).
    ...
    \item[Layer 5 - Hardware Abstraction (Pi4J + Adapters)] Interfaces between ROS 2 and physical hardware. Communicates with ESP32 via SPI (primary) and TCP (fallback). Reads LiDAR via COLA-B protocol.
...
\begin{tikzpicture}[
    ...
    % 15 Hz LiDAR Scan - every 66.67ms (not visible in 12ms window, show one)
    \draw[fill=purple!60] (0,2.2) rectangle (1.0,2.8);
    \node[label, left] at (-0.2,2.5) {LiDAR Scan};
    \node[freq, right] at (12.2,2.5) {15 Hz};
...
\begin{itemize}
    ...
    \item \textbf{15 Hz LiDAR} -- Fixed by TiM561 hardware. Sufficient for indoor navigation at walking speeds.
...
\begin{tikzpicture}[
    ...
    % LiDAR
    \node[box, fill=purple!20] (lidar) {SICK TiM561};

    % sick_scan_xd
    \node[box, fill=blue!20, right=1.5cm of lidar] (driver) {sick\_scan\_xd};
    ...
    % Arrows
    \draw[arrow] (lidar) -- node[above, font=\tiny] {COLA-B\\TCP} (driver);
    ...
    \node[font=\tiny, gray, below=0.1cm of lidar] {15 Hz};
...
\begin{tikzpicture}[
    ...
    \draw[arrow] (lidar) -- node[above, font=\tiny] {TCP} (agg);
...
\begin{enumerate}
    ...
    \item[$\square$] Implement \texttt{SickLidarAdapter} (COLA-B protocol)
