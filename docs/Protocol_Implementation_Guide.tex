...
\pagestyle{fancy}
\fancyhf{}
\rhead{STAR Protocol Implementation}
\lhead{Implementation Guide}
\cfoot{\thepage}
...
% Document metadata
\title{\textbf{STAR SPI Communication Protocol:\\Implementation Guide}\\[0.5em]\large{nanopb, ARQ, and CRC-32 for ESP32-S3 Motor Control}}
...
\begin{abstract}
This document provides a comprehensive implementation guide for the STAR firmware SPI communication protocol. The protocol uses Protocol Buffers (nanopb) for serialization, Stop-and-Wait ARQ for reliability, and hardware-accelerated CRC-32 for error detection. All specifications are derived from the design document \texttt{Protobuf\_Protocol\_Design\_Analysis.tex}.

The protocol achieves: (1) 99.9999\% reliability with CRC-32, (2) 27× throughput margin at 10~Mbps SPI with Stop-and-Wait ARQ, (3) <10~KB memory footprint with zero dynamic allocation via nanopb, and (4) <20~ms emergency stop latency for safety-critical motor control.
\end{abstract}
