...
\textbf{Overview:} Protocol Buffers (nanopb) encapsulated in frames with CRC-32 and Automatic Repeat Request (ARQ) for reliable SPI communication between RPi5 and RX72N.
...
\noindent\colorbox{layer3color}{\parbox{\dimexpr\linewidth-2\fboxsep}{%
\textbf{Layer 3: ARQ Layer} \textit{(Custom Code - SAME AS RX72N)}
\vspace{0.1cm}
...
\begin{enumerate}[leftmargin=1.5em,topsep=4pt,itemsep=2pt]
    ...
    \item \textbf{RX72N Frame:} Verify CRC-32 $\rightarrow$ valid
    \item \textbf{RX72N ARQ:} Check \texttt{seq=42} = \texttt{rx\_last\_ack+1} $\rightarrow$ accept
    ...
    \item \textbf{RX72N ARQ:} Build ACK packet with \texttt{ack\_seq=42}
