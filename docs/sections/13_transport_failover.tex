...
\begin{itemize}[leftmargin=1.5em,topsep=2pt,itemsep=1pt]
    \item \textbf{Hard failure} (cable pull, IO error): $<$ 300\,ms (no drain, immediate switch)
    \item \textbf{Graceful/timeout}: $<$ 700\,ms (500\,ms drain + 200\,ms switch overhead)
    \item \textbf{Silent failure} (PING timeout): Up to 1.5\,s detection + switch time
\end{itemize}
For silent failures exceeding 500\,ms, the RX72N watchdog \textbf{will trigger}, initiating safe motor shutdown. The gateway detects this via health monitoring and resumes communication on the new transport. This is the intended fail-safe behavior: motor safety takes precedence over seamless failover.
}}
