...
\begin{itemize}
\item \textbf{SYNC}: Magic number \texttt{0x55AA} for frame synchronization
\item \textbf{SEQ}: Sequence number (0-65535, wraps around)
\item \textbf{LEN}: Payload length in bytes
\item \textbf{TYPE}: Frame type (see Table~\ref{tab:frame_types})
\item \textbf{FLAGS}: Control flags (RequiresAck, HasFEC, etc.)
\item \textbf{PAYLOAD}: Variable-length data (Protocol Buffer message)
\end{itemize}

\textbf{Checksum (little-endian, IEEE 802.3 LSB-first):}
...
RESET\_ACK & 0xFE & 0 & (empty) & 0x110062D8 \\
\bottomrule
\end{tabular}
\caption{Cross-Compatibility Test Vectors}
\label{tab:cross_compat_vectors}
\end{table}

\textbf{Test locations:}
\begin{itemize}
\item Go: \texttt{star-gateway/internal/frame/frame\_test.go}
\item C: \texttt{e2-studio-star-rx72n-firmware/tests/test\_rx\_frame.c}
\end{itemize}

\subsection{USB CDC Lightweight Protocol}

The USB CDC protocol is designed for minimal overhead, relying on USB hardware for reliability. Unlike the SPI path which uses full HARQ with FEC and retransmission, the CDC path omits these as redundant with USB's built-in CRC and retry mechanisms.
...
\begin{itemize}
\item \checkmark{} Sequence tracking via shared SessionState
\item \checkmark{} CRC32 validation
...
\label{tab:heartbeat_timing}
\end{table}

\subsubsection{Control Frame Dispatching}

Both firmware (C) and gateway (Go) auto-respond PONG to PING and RESET\_ACK to RESET. Control frames (PING, PONG, ACK, NACK, RESET\_ACK) are consumed internally. Only data frames (COMMAND, RESPONSE) are returned to callers.
...
\begin{tikzpicture}[node distance=2.5cm, auto, every node/.style={align=center}]
  ...
  \node[rectangle, draw, right of=usb_init, text width=2cm] (enum) {Enumeration\\(0-200ms)};
  \node[rectangle, draw, below of=enum] (configured) {Configured};
  \node[rectangle, draw, left of=configured] (flush) {Flush Buffer};
  \node[rectangle, draw, left of=flush, text width=1.8cm] (normal) {Normal\\Logging};
...
Latency (send 1KB) & 0.5-1 ms & rx\_usb\_write() to host receive \\
Latency (receive 1KB) & 0.5-1 ms & Host write to rx\_usb\_read() \\
\hline
\end{tabular}
...
\item \textbf{Transport Architecture}: \texttt{star-gateway/TRANSPORT\_ARCHITECTURE.md}
\item \textbf{Implementation}: \texttt{libs/rx\_usb/inc/rx\_usb.h}, \texttt{libs/rx\_core/src/rx\_log\_usb.c}
\item \textbf{Cross-Compatibility Tests}: \texttt{star-gateway/internal/frame/frame\_test.go}, \texttt{e2-studio-star-rx72n-firmware/tests/test\_rx\_frame.c}
