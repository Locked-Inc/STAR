...
The gateway implements all layers of the communication protocol (see Section~\ref{sec:protocol-stack}).
...
|   |   +-- main.go              # Entry point
|   +-- virtual_rx72n/
|       +-- main.go              # RX72N simulator
+-- internal/
|   +-- transport/               # Layer 1: Physical transport
|   |   +-- spi.go               # SPI transport (periph.io)
|   |   +-- cdc.go               # USB CDC transport
|   |   +-- socket.go            # Socket transport (simulation)
|   +-- frame/                   # Layer 2: Frame protocol
|   |   +-- frame.go             # Constants and types
|   |   +-- encoder.go           # Frame encoder
|   |   +-- decoder.go           # Stream decoder
|   +-- link/                    # Layer 3: Reliability (HARQ)
|   |   +-- spi.go               # SPILink (HARQ + FEC)
|   +-- harq/                    # HARQ interface and types
|   |   +-- harq.go
|   +-- fec/                     # FEC codec
|   |   +-- convolutional.go     # Rate 1/2, K=7 encoder
|   |   +-- viterbi.go           # Viterbi decoder
|   |   +-- combiner.go          # Chase Combiner
...
\hline
SPI & \texttt{/dev/spidev0.0} & 10 MHz & Backup (priority 5) \\
USB CDC & \texttt{/dev/ttyACM0} & 12 Mbps (Full-Speed) & Primary (priority 10) \\
...
\subsection{Link Layer (HARQ + FEC)}
...
\begin{itemize}[leftmargin=1.5em,topsep=4pt,itemsep=2pt]
    \item \textbf{HARQ with ACK/NACK} -- Retransmission on negative acknowledgment or timeout
    \item \textbf{FEC encoding/decoding} -- Convolutional code (K=7, Rate 1/2) with Viterbi decoding
    \item \textbf{Chase Combining} -- Soft-combining of retransmitted frames for improved error recovery
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em,topsep=4pt,itemsep=2pt]
    \item \textbf{No HARQ} -- USB protocol provides its own error correction and retransmission
    \item \textbf{Sequence tracking} -- Maintains sequence numbers for ordering
\end{itemize}

\subsubsection{Shared Session State}

Both link types use the shared \texttt{SessionState} from \texttt{internal/manager/} to maintain sequence number continuity across transport failovers.
...
The \texttt{internal/manager} package provides priority-based transport selection and automatic failover.
...
\begin{itemize}[leftmargin=1.5em,topsep=4pt,itemsep=2pt]
    \item \textbf{Shared SessionState} -- Sequence continuity maintained across transports
    \item \textbf{Automatic failover} -- Health monitoring detects transport failures
    \item \textbf{Primary timeout} -- 200 ms implicit timeout before failover
    \item \textbf{Secondary probe} -- 1 s PING probe to detect recovery of higher-priority transport
\end{itemize}
...
\noindent For the complete end-to-end communication path from UI to motor controller, see Section~\ref{sec:communication-stack}.
