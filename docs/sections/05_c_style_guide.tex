...
\begin{itemize}
  \item \emph{Static Allocation}: All buffers allocated at compile time (no malloc)
  \item \emph{Small Code Size}: Minimal flash/RAM footprint
  \item \emph{C Language}: Compatible with C99 (optional C11 features; not C++)
  \item \emph{Field Size Constraints}: Maximum sizes configured in \texttt{.options} files
\end{itemize}
