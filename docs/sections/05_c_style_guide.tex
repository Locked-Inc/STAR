...
\begin{lstlisting}[language=C]
/* Forward declaration (C23 or later with explicit underlying type) */
enum star_bus_type : int;
...
Logging is handled using platform-specific logging macros (e.g., \texttt{RX\_LOG\_*} for STAR, \texttt{ESP\_LOG*} for ESP-IDF compatibility layer).
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{C Language}: Compatible with C23 (not C++ required)
