...
This section defines comprehensive style guidelines for C firmware development in the STAR project. All firmware code (ESP32-S3, RX72N, and future platforms) must follow these conventions to ensure consistency, maintainability, and safety in embedded systems development.
...
Only use \texttt{char} for actual character strings, never for byte data.
...
Use \texttt{uint32\_t} or \texttt{size\_t} for loop counters and array indices.
...
\subsubsection{Summary: Type Usage Rules}

\begin{itemize}
    \item \textbf{ALWAYS}: Use \texttt{uint8\_t}, \texttt{int32\_t}, etc. from \texttt{<stdint.h>}
    \item \textbf{NEVER}: Use \texttt{int}, \texttt{unsigned}, \texttt{short}, \texttt{long}
    \item \textbf{ONLY}: Use \texttt{char} for actual character strings
    \item \textbf{USE}: \texttt{bool} from \texttt{<stdbool.h>} for boolean values
    \item \textbf{USE}: \texttt{size\_t} for sizes and counts from sizeof()
...
star_encoder_get_velocity_rpm(&encoder, 10.0f, 500, &current_velocity_mps);
...
#define MOTOR_THERMAL_SHUTDOWN_CELSIUS  (85.0f)

if (motor_temp_celsius > MOTOR_THERMAL_SHUTDOWN_CELSIUS) {
...
star_drv8243_read_current(&motor, &motor_current_ma);
...
/* Current limiting */
#define MAX_MOTOR_CURRENT_MA  (2000)  /* 2A limit */

if (motor_current_ma > MAX_MOTOR_CURRENT_MA) {
  ESP_LOGW(s_TAG, "Motor current limit exceeded: %lu mA", motor_current_ma);
  star_drv8243_set_speed(&motor, 0.0f);
}
...
star_drv8243_set_speed(&motor, duty_cycle_percent);
...
This section establishes memory management rules for ESP32 firmware. The ESP32-S3 has limited RAM (512KB SRAM, no PSRAM in our configuration), making proper memory management critical for system stability and performance.
...
\end{lstlisting}
\subsubsection{ESP-IDF Heap Functions}

ESP-IDF provides heap functions (\texttt{heap\_caps\_malloc}, \texttt{esp\_heap\_caps\_malloc}) that are \textbf{only acceptable} for:
...
esp_err_t sensor_driver_init(sensor_handle_t* handle)
{
  /* Allocate persistent buffer once */
  handle->rx_buffer = heap_caps_malloc(DMA_BUFFER_SIZE,
  MALLOC_CAP_DMA);
...
esp_err_t sensor_driver_deinit(sensor_handle_t* handle)
{
...
\begin{itemize}
\item ESP32 default task stack: \textbf{3KB--8KB} (configurable in FreeRTOS)
...
\subsection{Use \texttt{const} for Flash Storage}
...
\begin{itemize}
\item ESP32 has \textbf{16MB flash} but only \textbf{512KB RAM}
...
void receive_packet(const uint8_t* data, size_t len)
{
  if (data == NULL || len > RX_BUFFER_SIZE) {
    ESP_LOGE(s_TAG, "Invalid packet: len=%zu (max=%d)",
    len, RX_BUFFER_SIZE);
    return;
  }
...
size_t* bytes_copied)
{
  ...
  *bytes_copied = copy_len;
  ...
  if (len > RX_BUFFER_SIZE) {
    ESP_LOGW(s_TAG, "Packet truncated: %zu bytes lost",
    len - RX_BUFFER_SIZE);
  }
}
...
esp_err_t start_dma_transfer(const uint8_t* src, size_t len)
{
  ...
  if (len == 0 || len > MAX_DMA_TRANSFER_SIZE) {
    ESP_LOGE(s_TAG, "Invalid DMA length: %zu", len);
    return ESP_ERR_INVALID_SIZE;
  }
  
  /* Check alignment (DMA requires 4-byte alignment) */
  if (((uintptr_t)src & 0x3) != 0) {
    ESP_LOGE(s_TAG, "DMA buffer not 4-byte aligned");
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t ring_buffer_write(ring_buffer_t* rb, const uint8_t* data,
size_t len)
{
  ...
  if (len > available) {
    ESP_LOGW(s_TAG, "Ring buffer overflow: %zu bytes lost",
    len - available);
    return ESP_ERR_NO_MEM;
  }
...
This section establishes best practices for real-time embedded systems development on ESP32. These guidelines are critical for motor control (1000Hz loops), sensor fusion, and safety-critical robotics applications.
...
\item \textbf{FromISR variants}: Use \texttt{xQueueSendFromISR()}, not \texttt{xQueueSend()}
...
ESP_LOGI(s_TAG, "Execution time: %lu us", elapsed_us);
...
/* If task hangs, watchdog resets ESP32 after 5 seconds */
...
esp_err_t read_temperature_safe(float* temp_celsius)
{
  esp_err_t ret;
  uint8_t data[2];
  
  /* I2C read with 100ms timeout */
  ret = star_bus_i2c_read(bus_manager, "temp_i2c", data, 2,
  TEMP_READ_REG, NULL);
  if (ret != ESP_OK) {
    ESP_LOGW(s_TAG, "Temperature sensor timeout");
    return ret;
  }
...
esp_err_t ret = read_temperature_safe(&motor_temp);
if (ret != ESP_OK) {
  /* Sensor failed - enter safe state */
  star_drv8243_stop(&motor, true);
...
{
  ESP_LOGE(s_TAG, "ENTERING SAFE MODE");
  
  /* Stop all motors */
  for (int i = 0; i < NUM_MOTORS; i++) {
    star_drv8243_stop(&motors[i], true);  /* Brake */
  }
...
if (battery_voltage_mv < BATTERY_CRITICAL_MV) {
  system_enter_safe_mode();
}

if (motor_temp_celsius > THERMAL_SHUTDOWN_CELSIUS) {
  system_enter_safe_mode();
}
...
esp_err_t save_config_to_nvs(const star_pid_config_t* config)
{
  stored_config_t stored;
  memcpy(&stored.pid, config, sizeof(star_pid_config_t));
  
  /* Calculate CRC-32 */
  stored.crc32 = esp_crc32_le(0, (uint8_t*)&stored.pid,
  sizeof(star_pid_config_t));
...
esp_err_t load_config_from_nvs(star_pid_config_t* config)
{
  stored_config_t stored;
  size_t len = sizeof(stored_config_t);
  
  esp_err_t ret = nvs_get_blob(nvs_handle, "pid_config",
  &stored, &len);
  if (ret != ESP_OK) {
    return ret;
  }
  ...
  /* Verify CRC */
  uint32_t calculated_crc = esp_crc32_le(0, (uint8_t*)&stored.pid,
  sizeof(star_pid_config_t));
  if (calculated_crc != stored.crc32) {
    ESP_LOGE(s_TAG, "Config CRC mismatch! Flash corrupted?");
...
\begin{lstlisting}[language=C]
/* CORRECT - Driver depends on bus abstraction, not ESP-IDF */
...
const char* bus_name)
{
  ...
  uint8_t whoami;
  esp_err_t ret = star_bus_i2c_read(bus_manager, bus_name,
  &whoami, 1, MPU6050_WHO_AM_I, NULL);
...
ESP_LOGE(s_TAG, "[%lu] Motor fault detected: current=%lu mA (limit=%d mA)",
...
\begin{lstlisting}[language=C]
/* I2C bus recovery */
esp_err_t recover_i2c_bus(i2c_port_t port)
{
...
\begin{itemize}
\item \textbf{Predictable timing} for motor control and sensor fusion
...
This section establishes configuration requirements for Protocol Buffer messages used in ESP32 firmware. The STAR project uses \textbf{nanopb} (Nano Protocol Buffers) for embedded systems, which requires static memory allocation.
...
\begin{itemize}
  \item \textbf{Static Allocation}: All buffers allocated at compile time (no malloc)
  \item \textbf{Small Code Size}: Minimal flash/RAM footprint
  \item \textbf{C Language}: Compatible with C99 (not C++ required)
  \item \textbf{Field Size Constraints}: Maximum sizes configured in \texttt{.options} files
\end{itemize}

\subsection{Why Static Allocation?}

ESP32 firmware follows strict "no dynamic allocation" rules (see Section 25). nanopb field sizes must be explicitly configured to enable compile-time buffer allocation.

\subsection{Field Size Configuration}

Field sizes are configured in \texttt{.options} files located alongside \texttt{.proto} files:

\begin{lstlisting}[language=bash]
# File: star-proto/proto/star.options
star.v1.RequestHeader.request_id max_size:64
star.v1.ResponseHeader.error_message max_size:128
star.v1.FirmwareChunk.data max_size:1024
...
\begin{table}[H]
\centering
\caption{nanopb Field Size Constraints for ESP32}
...
\hline
UUID (RFC 4122) & 64 bytes & \texttt{request\_id}, \texttt{session\_id} \\
\hline
Error messages & 128 bytes & \texttt{error\_message} \\
\hline
Version strings & 32 bytes & \texttt{firmware\_version} \\
\hline
Short names & 32 bytes & \texttt{component\_name} \\
\hline
Log messages & 256 bytes & \texttt{log\_text} \\
\hline
\end{tabular}
\end{table}
...
\hline
Battery cells (3S LiPo) & 3 & \texttt{cell\_mv[]} \\
\hline
Motors (quadruped) & 4 & \texttt{motor\_status[]} \\
\hline
Motors (hexapod) & 8 & \texttt{motor\_status[]} \\
\hline
Sensor array & 8--16 & \texttt{temperature\_sensors[]} \\
\hline
Telemetry samples & 32--64 & \texttt{velocity\_samples[]} \\
\hline
\end{tabular}
\end{table}
...
\textbf{Purpose} & \textbf{Size} & \textbf{Example} \\
\hline
Firmware chunk & 1024 bytes & \texttt{chunk\_data} \\
\hline
Image thumbnail & 4096 bytes & \texttt{thumbnail} \\
\hline
Small binary data & 256 bytes & \texttt{calibration\_blob} \\
\hline
Cryptographic key & 64 bytes & \texttt{aes\_key} \\
\hline
\end{tabular}
\end{table}
...
if (!status) {
  ESP_LOGE(s_TAG, "Encoding failed: %s", PB_GET_ERROR(&stream));
  return ESP_FAIL;
}

size_t message_length = stream.bytes_written;
ESP_LOGI(s_TAG, "Encoded message: %zu bytes", message_length);

/* Verify it fits in SPI transaction buffer */
if (message_length > SPI_MAX_TRANSFER_SIZE) {
  ESP_LOGE(s_TAG, "Message too large for SPI: %zu > %d",
  message_length, SPI_MAX_TRANSFER_SIZE);
  return ESP_ERR_INVALID_SIZE;
}
...
proto/
robot_command.proto     -> robot_command.options
battery_state.proto     -> battery_state.options
firmware_update.proto   -> firmware_update.options
...
Command.command_name max_size:32  # Field is 'name', not 'command_name'
...
Properly configured nanopb constraints enable reliable, predictable Protocol Buffer communication on resource-constrained ESP32 hardware.
