...
This section defines comprehensive style guidelines for C firmware development in the STAR project. All firmware code (ESP32, RX72N, and future platforms) must follow these conventions to ensure consistency, maintainability, and safety in embedded systems development.
...
\textbf{Note:} While platform-specific macros are useful during development, production code should use explicit error handling with proper cleanup and recovery mechanisms.
...
switch (curr->type) {
  ...
  default: {
    RX_LOG_WARN(manager->tag, "Skipping pin registration for "
...
*/
rx_err_t error_handler_record_error(error_handler_t* handler,
rx_err_t        error,
...
star_bus_config_t*  config)
{
  ...
  RX_RETURN_ON_FALSE(manager && config, RX_ERR_INVALID_ARG,
...
/* Usage */
RX_LOG_INFO(s_TAG, "Added bus config: %s (Type: %s)",
...
This section establishes error handling patterns for the STAR firmware codebase across all platforms (ESP32, RX72N, and future targets).
...
\begin{lstlisting}[language=C]
/* Platform-specific: ESP32 uses esp_check.h, RX72N uses rx_check.h */
#include "rx_check.h"
...
star_pin_interface_t*   pin_iface)
{
  ...
  RX_RETURN_ON_FALSE(manager->mutex,
  RX_ERR_INVALID_STATE,
  manager->tag,
...
size_t*                   bytes_written)
{
  RX_RETURN_ON_FALSE(manager && name && data,
  RX_ERR_INVALID_ARG,
  s_TAG,
...
star_bus_config_t*  config)
{
  ...
  RX_RETURN_ON_FALSE(manager->mutex, ESP_ERR_INVALID_STATE,
  ...
  pdMS_TO_TICKS(CONFIG_STAR_KCONFIG_BUS_MUTEX_TIMEOUT_MS))
  != pdTRUE) {
    RX_LOG_ERROR(manager->tag, "Timeout acquiring mutex for adding bus '%s'",
    config->name);
    return RX_ERR_TIMEOUT;
  }
...
typedef struct star_error_interface {
  rx_err_t (*record_error)(void* ctx, rx_err_t error,
...
typedef struct star_error_interface {
  rx_err_t (*record_error)(void*       ctx,
  rx_err_t   error,
  const char* message,
  const char* file,
  int         line,
...
typedef struct star_pin_interface {
  rx_err_t (*register_pin)(void* ctx, int pin,
  const char* description, bool shared);
  rx_err_t (*unregister_pin)(void* ctx, int pin,
  const char* description);
...
extern "C" {
  ...
  * @param[in]  error_iface Optional error handler interface (can be NULL).
  * @param[in]  pin_iface   Optional pin validator interface (can be NULL).
  * @return rx_err_t RX_OK on success, RX_ERR_INVALID_ARG if manager is NULL,
...
typedef struct star_spi_bus_config {
  ...
  * which we map here.
  */
  gpio_num_t copi_io_num;  /**< GPIO for COPI (maps to platform mosi_io_num) */
  gpio_num_t cipo_io_num;  /**< GPIO for CIPO (maps to platform miso_io_num) */
  /* ... */
} star_spi_bus_config_t;
...
This section establishes memory management rules for STAR firmware across all platforms. Embedded platforms have limited RAM (e.g., ESP32-S3: 512KB SRAM, RX72N: 1MB SRAM), making proper memory management critical for system stability and performance.
...
\begin{itemize}
\item Embedded platforms typically have large flash but limited RAM (e.g., ESP32: 16MB flash / 512KB RAM, RX72N: 4MB flash / 1MB RAM)
...
void receive_packet(const uint8_t* data, size_t len)
{
  if (data == NULL || len > RX_BUFFER_SIZE) {
    RX_LOG_ERROR(s_TAG, "Invalid packet: len=%zu (max=%d)",
    len, RX_BUFFER_SIZE);
    return;
  }
...
\begin{lstlisting}[language=C]
/* I2C bus recovery */
rx_err_t recover_i2c_bus(i2c_port_t port)
{
...
This section establishes configuration requirements for Protocol Buffer messages used in STAR embedded firmware (ESP32, RX72N). The STAR project uses \textbf{nanopb} (Nano Protocol Buffers) for embedded systems, which requires static memory allocation.
...
if (message_length > SPI_MAX_TRANSFER_SIZE) {
  RX_LOG_ERROR(s_TAG, "Message too large for SPI: %zu > %d",
...
Properly configured nanopb constraints enable reliable, predictable Protocol Buffer communication on resource-constrained embedded hardware (ESP32, RX72N, and future platforms).
