...
/* STAR PCB: 24 MHz crystal feeding PLL (PLL_MUL=10 -> 240 MHz).
 * Confirmed against star-rx72n-firmware/src/rx_clock_power_init.c, the
 * known-good motor_spin_test/clock.c, and uart_test/clock.c -- all say
 * "24 MHz external crystal". The "12 MHz" claim in rx72n_clock.h's
 * Doxygen comment is stale; the working code wins per CLAUDE.md
 * "Trust headers over comments / Trust known-good bench tests" rules. */
...
*/
/* STAR PCB: 24 MHz * 10 = 240 MHz PLL output. STC encoding written by
 * mcu_clocks.c is ((PLL_MUL*2) - 1) = 19, matching the working HAL's
 * k_pllcr_mosc_x10 = 0x1300 (STC field = 0x13 = 19). */
