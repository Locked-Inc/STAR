...
/* RX72N port numbers used by the v0 bring-up.  Only the ports we
 * actually touch from this TU are listed; add more as needed.
 *
 ...
 * Motor IN1/IN2 -> GPTW GTIOCnA/GTIOCnB.        plan/pinout.tex
 *   NOTE: PSEL value for GPTW GTIOC pins varies per pin; verify each
 *   against HW Ch.23 Table 23.6 before promoting this code from
 *   "untested".  For v0 we stub them out and only initialise SCI/RIIC --
 *   motor PWM init lives in motors.c which checks an `armed` flag at
 *   runtime. */
typedef enum : uint8_t {
    k_port_p2 = 0x02U,
    k_port_p3 = 0x03U,
...
/* Pin numbers within their respective ports (named, not raw). */
typedef enum : uint8_t {
    k_pin_sda1    = 0U, /* P20 -- RIIC1 SDA   */
    k_pin_scl1    = 1U, /* P21 -- RIIC1 SCL   */
    k_pin_imu_int = 2U, /* P32 -- BNO055 INT  */
    k_pin_imu_rst = 3U, /* P83 -- BNO055 nRST */
} pin_num_t;
...
static const motor_ctrl_pins_t s_motor_pins[4] = {
    ...
    {.nsleep_port = k_port_p6, .nsleep_pin = 0U, .drvoff_port = k_port_p6, .drvoff_pin = 1U},
    /* Motor 1: P62 nSLEEP, P63 DRVOFF */
    {.nsleep_port = k_port_p6, .nsleep_pin = 2U, .drvoff_port = k_port_p6, .drvoff_pin = 3U},
    /* Motor 2: P64 nSLEEP, PE0 DRVOFF */
    {.nsleep_port = k_port_p6, .nsleep_pin = 4U, .drvoff_port = k_port_pe, .drvoff_pin = 0U},
    /* Motor 3: PE1 nSLEEP, PE2 DRVOFF */
    {.nsleep_port = k_port_pe, .nsleep_pin = 1U, .drvoff_port = k_port_pe, .drvoff_pin = 2U},
};
...
void hw_init_board(void)
{
    ...
    pfs_write(k_port_p2, k_pin_sda1, k_psel_riic1);
    pfs_write(k_port_p2, k_pin_scl1, k_psel_riic1);
    ...
    pin_to_peripheral(k_port_p2, k_pin_sda1);
    pin_to_peripheral(k_port_p2, k_pin_scl1);

    /* IMU reset (P83) as output, idle high (deasserted). Pulse done by
     * hw_pulse_imu_reset() called from main(). imu_test/main.c:107-114. */
    gpio_set_output(k_port_p8, k_pin_imu_rst);
    gpio_write(k_port_p8, k_pin_imu_rst, true);

    /* IMU INT (P32) as input with pullup. Polled in v0. */
    gpio_set_input(k_port_p3, k_pin_imu_int, true);
...
void hw_pulse_imu_reset(void)
{
    ...
    gpio_write(k_port_p8, k_pin_imu_rst, false);
    for (volatile uint32_t i = 0; i < 200000U; i++) {
        __asm__ volatile("nop");
    }
    gpio_write(k_port_p8, k_pin_imu_rst, true);
...
void hw_motor_set_nsleep(uint8_t motor_idx, bool enabled)
{
    ...
    gpio_write(s_motor_pins[motor_idx].nsleep_port, s_motor_pins[motor_idx].nsleep_pin, enabled);
}
...
void hw_motor_set_drvoff(uint8_t motor_idx, bool disabled)
{
    ...
    gpio_write(s_motor_pins[motor_idx].drvoff_port, s_motor_pins[motor_idx].drvoff_pin, disabled);
}
