...
static const char* TAG = "STAR_ASYNC";
...
typedef struct async_operation {
  ...
  uint8_t               priority;       /**< Operation priority */
...
/**
 * @brief Initialize global async state
 */
 ...
 */
static async_state_t* get_async_state(const char* bus_name)
{
  init_global_state();
  ...
  for (uint8_t i = 0; i < g_num_async_states; i++) {
    /* TODO: Store bus name for comparison */
  }
 ...
 */
static esp_err_t enqueue_operation(async_state_t* state, async_operation_t* op)
{
 ...
 */
static void execute_operation(async_operation_t* op)
{
  if (op == NULL) {
    return;
  }
  ...
  switch (op->type) {
    ...
    default:
      ESP_LOGE(TAG, "Unknown operation type: %d", op->type);
 ...
 */
static void async_worker_task(void* param)
{
  async_state_t* state = (async_state_t*)param;

  ESP_LOGI(TAG, "Async worker task started");
...
/**
 * @brief Ensure worker task is running
 */
static esp_err_t ensure_worker_running(async_state_t* state)
{
  if (state == NULL) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (!state->worker_running) {
    ...
    BaseType_t created =
      xTaskCreate(async_worker_task, "async_worker", 4096, state, 5, &state->worker_task);
                                   ...
                                   star_async_handle_t*       handle)
{
  ...
  async_state_t* state = get_async_state(bus_name);
  if (state == NULL) {
    return ESP_ERR_NO_MEM;
  }

  esp_err_t result = ensure_worker_running(state);
  if (result != ESP_OK) {
    return result;
  }
  ...
  result = enqueue_operation(state, op);
  if (result != ESP_OK) {
    free(op);
    return result;
  }
