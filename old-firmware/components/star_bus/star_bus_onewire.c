...
static const char* TAG = "STAR_ONEWIRE";
...
/**
 * @brief Get 1-Wire state for a bus
 */
                                ...
                                const star_onewire_config_t* config)
{
  ...
  ESP_LOGI(TAG, "1-Wire bus '%s' initialized on GPIO %d", bus_name, config->gpio_pin);
...
esp_err_t star_bus_onewire_deinit(star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "1-Wire bus '%s' deinitialized", bus_name);
                                  ...
                                  size_t*             count)
{
  ...
  *count             = 0;
  ...
  ESP_LOGI(TAG, "Found %d device(s) on bus '%s'", *count, bus_name);
...
esp_err_t star_bus_onewire_reset_stats(star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(TAG, "Statistics reset for 1-Wire bus '%s'", bus_name);
