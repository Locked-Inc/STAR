#!/usr/bin/env bash
# esp32-firmware/add_file_header.sh
...
# Check if we're in a git repository and get paths relative to git root
if git rev-parse --show-toplevel &> /dev/null; then
    GIT_ROOT=$(git rev-parse --show-toplevel)
    CURRENT_DIR=$(pwd)
    # Calculate the relative path from git root to current directory
    REL_PATH_FROM_ROOT="${CURRENT_DIR#$GIT_ROOT/}"
    USE_GIT_PATHS=true
else
    USE_GIT_PATHS=false
fi
...
while IFS= read -r file; do
    ...
    # If in a git repo, use path relative to git root; otherwise use current directory
    if [ "$USE_GIT_PATHS" = true ]; then
        # Remove leading "./" and prepend git-relative path
        local_path="${file#./}"
        if [ "$REL_PATH_FROM_ROOT" = "$CURRENT_DIR" ]; then
            # We're at the git root
            filepath="$local_path"
        else
            # We're in a subdirectory
            filepath="${REL_PATH_FROM_ROOT}/${local_path}"
        fi
    else
        # Not in git, use current directory relative path
        filepath="${file#./}"
    fi
