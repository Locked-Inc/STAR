<script>
  ...
  $: filteredRawPackets = rawPackets.filter(packet => {
    if (rawFilter === 'tx') return packet.direction === 'Tx';
    if (rawFilter === 'rx') return packet.direction === 'Rx';
    return true;
  }).filter(packet => {
  ...
  $: filteredParsedPackets = parsedPackets.filter(packet => {
    if (parsedFilter === 'tx') return packet.direction === 'Tx';
    if (parsedFilter === 'rx') return packet.direction === 'Rx';
    return true;
  }).filter(packet => {
  ...
  async function fetchPackets() {
    try {
      if (activeTab === 'raw') {
        const packets = await invoke('get_raw_packets', { limit: 1000, offset: 0 });
        rawPackets = packets;
      } else {
        const packets = await invoke('get_parsed_packets', { limit: 1000, offset: 0 });
  ...
  function handleMouseMove(event) {
    ...
    const newHeight = Math.max(
      MIN_HEIGHT,
      Math.min(startHeight + deltaY, window.innerHeight * MAX_HEIGHT_PERCENT)
    );
    height = newHeight;
    localStorage.setItem('bottomPanelHeight', height.toString());
  }
  ...
  async function exportToFile() {
    const content = filteredRawPackets.map(packet => {
    ...
    a.href = url;
    a.download = `bms-packets-${Date.now()}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  }

  // PARSED view helper functions
  function toggleFieldExpansion(packetIndex, path) {
    const key = `${packetIndex}-${path}`;
    expandedFields[key] = !expandedFields[key];
    expandedFields = expandedFields; // Trigger reactivity
  }

  function isFieldExpanded(packetIndex, path) {
    const key = `${packetIndex}-${path}`;
  ...
  function handleParsedSelect(index) {
    if (selectedParsedIndex === index) {
      selectedParsedIndex = null;
    } else {
      selectedParsedIndex = index;
      // Reset expanded fields when selecting new packet
      expandedFields = {};
    }
  }

  async function copyParsedData() {
    if (selectedParsedIndex === null) return;
    const packet = filteredParsedPackets[selectedParsedIndex];
    const jsonData = JSON.stringify(packet.fields, null, 2);
    try {
  ...
  async function exportParsedToFile() {
    const content = JSON.stringify(filteredParsedPackets, null, 2);
    const blob = new Blob([content], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `bms-parsed-packets-${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }

  function renderJsonTree(obj, packetIndex, path = '') {
    ...
    if (Array.isArray(obj)) {
      ...
      const isExpanded = isFieldExpanded(packetIndex, path);
      ...
      if (isExpanded) {
        ...
        obj.forEach((item, index) => {
          ...
          html += renderJsonTree(item, packetIndex, itemPath);
          html += `</div>`;
        });
        html += `</div>`;
      } else {
    ...
    if (isExpanded) {
      ...
      keys.forEach(key => {
        ...
        html += renderJsonTree(value, packetIndex, fieldPath);
        html += `</div>`;
      });
      html += `</div>`;
    } else {
  ...
  function addConsoleOutput(type, text) {
    ...
    consoleOutput = [...consoleOutput, { timestamp, type, text }];
  ...
  async function copyConsole() {
    const text = consoleOutput.map(line => `[${line.timestamp}] ${line.text}`).join('\n');
    try {
...
{#if visible}
  <div class="bottom-panel" style="height: {height}px">
    ...
    <div
      ...
      aria-valuemax={Math.floor(window.innerHeight * MAX_HEIGHT_PERCENT)}
      tabindex="0"
      ...
      {#if activeTab === 'raw'}
        <div class="raw-view-enhanced">
          <div class="raw-toolbar">
            ...
            <div class="action-group">
              <button
                ...
                disabled={selectedPacketIndex === null}
          ...
          <div class="packet-list">
            {#each filteredRawPackets as packet, index}
              <div
                class="packet-item {packet.direction.toLowerCase()}"
                class:selected={selectedPacketIndex === index}
                on:click={() => handlePacketSelect(index)}
                role="button"
                tabindex="0"
                on:keydown={(e) => e.key === 'Enter' && handlePacketSelect(index)}
              >
      ...
      {:else if activeTab === 'parsed'}
        <div class="parsed-view-enhanced">
          <div class="parsed-toolbar">
            ...
            <div class="action-group">
              <button
                ...
                disabled={selectedParsedIndex === null}
          ...
          <div class="parsed-list">
            {#each filteredParsedPackets as packet, index}
              <!-- svelte-ignore a11y_no_noninteractive_element_interactions -->
              <div
                class="parsed-item {packet.direction.toLowerCase()}"
                class:selected={selectedParsedIndex === index}
                on:click={() => handleParsedSelect(index)}
                role="button"
                tabindex="0"
                on:keydown={(e) => e.key === 'Enter' && handleParsedSelect(index)}
              >
                ...
                {#if selectedParsedIndex === index}
                  <div class="parsed-details">
                    ...
                    <div class="json-viewer" on:click={(e) => {
                      const expandable = e.target.closest('.json-expandable');
                      if (expandable) {
                        const path = expandable.getAttribute('data-path');
                        toggleFieldExpansion(index, path);
                        e.stopPropagation();
                      }
                    }}>
                      {@html renderJsonTree(packet.fields, index)}
                    </div>
                  </div>
                {/if}
              </div>
...
<style>
  ...
  .json-expandable:hover > .json-toggle {
    color: var(--accent-primary, #2563EB);
  }
