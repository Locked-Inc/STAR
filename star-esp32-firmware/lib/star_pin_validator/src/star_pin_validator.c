...
esp_err_t star_register_pin(gpio_num_t gpio_num, const char* desc, bool can_be_shared)
{
  ...
  if (strnlen(desc, k_pin_validator_desc_max_len) >= k_pin_validator_desc_max_len) {
    ESP_LOGE(s_tag, "Description too long (max: %d characters)", k_pin_validator_desc_max_len - 1);
    return ESP_ERR_INVALID_ARG;
  }
