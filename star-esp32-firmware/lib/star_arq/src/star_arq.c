...
static esp_err_t internal_wait_for_ack(star_arq_handle_t* handle,
...
esp_err_t star_arq_init(star_arq_handle_t* handle, const star_arq_config_t* config)
{
  ...
  handle->max_retries =
    (config->max_retries > 0) ? config->max_retries : k_star_arq_default_max_retries;
  handle->retry_timeout_ms =
    (config->retry_timeout_ms > 0) ? config->retry_timeout_ms : k_star_arq_default_timeout_ms;
...
star_arq_send(star_arq_handle_t* handle, const uint8_t* data, size_t len, uint32_t timeout_ms)
{
  ...
  esp_err_t ret = star_frame_build(&handle->tx_frame,
                                   handle->tx_seq,
  ...
  for (handle->retry_count = 0; handle->retry_count <= handle->max_retries; handle->retry_count++) {
    /* Send frame */
    ret = star_bus_spi_peripheral_transmit(handle->bus_manager,
                                           handle->spi_bus_name,
                                           handle->retry_buffer,
    ...
    if (ret != ESP_OK) {
      ESP_LOGW(s_tag,
               "SPI transmit failed (attempt %u/%u): %s",
    ...
    ESP_LOGD(s_tag,
             "Sent request: seq=%u, len=%zu (attempt %u/%u)",
             handle->tx_seq,
             len,
             handle->retry_count + 1,
             handle->max_retries + 1);
                           ...
                           uint32_t           timeout_ms)
{
  ...
  while (true) {
    ...
    esp_err_t ret = star_bus_spi_peripheral_receive(handle->bus_manager,
                                                    handle->spi_bus_name,
                                                    rx_buffer,
                                                    sizeof(rx_buffer),
                                                    timeout_ms);
    ...
    ret = star_frame_parse(&handle->rx_frame,
                           rx_buffer,
                           sizeof(rx_buffer),
                           &seq,
                           &type,
