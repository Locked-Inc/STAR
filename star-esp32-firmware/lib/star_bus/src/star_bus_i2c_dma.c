...
static const char* s_TAG = "STAR_I2C_DMA";
...
static i2c_port_t get_i2c_port_from_bus(star_bus_manager_t* manager, const char* bus_name)
{
  ...
  if (config == NULL || config->type != k_star_bus_type_i2c) {
    return I2C_NUM_MAX; /* Invalid */
  }

  return config->proto.i2c.port;
}
                                     ...
                                     const star_i2c_dma_config_t* config)
{
  ...
  if (port == I2C_NUM_MAX) {
    ESP_LOGE(s_TAG, "Bus '%s' not found or not I2C", bus_name);
  ...
  if (config->dma_channel >= 2) { /* ESP32 has 2 DMA channels */
    ESP_LOGE(s_TAG, "Invalid DMA channel: %d", config->dma_channel);
    return ESP_ERR_INVALID_ARG;
  }
                                 ...
                                 void*                   context)
{
  ...
  if (!star_i2c_is_dma_capable(data, length)) {
    ESP_LOGW(s_TAG, "Buffer not DMA-capable, falling back to standard I2C");
    esp_err_t result = star_bus_i2c_write(manager, bus_name, data, length, command, NULL);
  ...
  ESP_LOGD(s_TAG, "Using DMA for write: %zu bytes", length);
                                ...
                                void*                   context)
{
  ...
  ESP_LOGD(s_TAG, "Using DMA for read: %zu bytes", length);
