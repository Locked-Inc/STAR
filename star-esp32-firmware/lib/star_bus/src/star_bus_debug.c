                                  ...
                                  size_t*                   num_found)
{
  ...
  if (config == NULL) {
    ESP_LOGE(s_TAG, "Bus '%s' not found", bus_name);
    return ESP_ERR_NOT_FOUND;
  }

  if (config->type != k_star_bus_type_i2c) {
    ESP_LOGE(s_TAG, "Bus '%s' is not an I2C bus", bus_name);
    return ESP_ERR_INVALID_ARG;
  }

  size_t found_count = 0;

  ESP_LOGI(s_TAG, "Scanning I2C bus '%s'...", bus_name);
  ...
  ESP_LOGI(s_TAG, "Scan complete. Found %zu device(s)", found_count);
  return ESP_OK;
}
...
esp_err_t star_bus_debug_print_buses(const star_bus_manager_t* manager)
{
  ...
  ESP_LOGI(s_TAG, "=== Configured Buses ===");
  ...
  ESP_LOGI(s_TAG, "Total: %d bus(es) configured", count);
  return ESP_OK;
}
...
esp_err_t star_bus_debug_print_bus_info(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(s_TAG, "=== Bus Info: %s ===", bus_name);

  const char* type_str = star_bus_type_to_string(config->type);

  ESP_LOGI(s_TAG, "  Type: %s", type_str);
  ESP_LOGI(s_TAG, "  Initialized: %s", config->initialized ? "Yes" : "No");

  if (config->type == k_star_bus_type_i2c) {
    ESP_LOGI(s_TAG, "  I2C Address: 0x%02X", config->proto.i2c.address);
    ESP_LOGI(s_TAG, "  I2C Port: %d", config->proto.i2c.port);
    ESP_LOGI(s_TAG, "  SDA Pin: %d", config->proto.i2c.config.sda_io_num);
    ESP_LOGI(s_TAG, "  SCL Pin: %d", config->proto.i2c.config.scl_io_num);
    ESP_LOGI(s_TAG, "  Clock Speed: %" PRIu32 " Hz", config->proto.i2c.config.master.clk_speed);
  }
...
star_bus_debug_read_reg(const star_bus_manager_t* manager, const char* bus_name, uint8_t reg_addr)
{
  ...
  if (ret == ESP_OK && bytes_read == 1) {
    ESP_LOGI(s_TAG, "Read register 0x%02X: 0x%02X", reg_addr, value);
  } else {
    ESP_LOGE(s_TAG, "Failed to read register 0x%02X: %s", reg_addr, esp_err_to_name(ret));
  }
                                   ...
                                   uint8_t                   value)
{
  ...
  if (ret == ESP_OK && bytes_written == 1) {
    ESP_LOGI(s_TAG, "Write register 0x%02X: 0x%02X", reg_addr, value);
  } else {
    ESP_LOGE(s_TAG, "Failed to write register 0x%02X: %s", reg_addr, esp_err_to_name(ret));
  }
                                   ...
                                   size_t                    count)
{
  ...
  for (size_t i = 0; i < max_read; i++) {
    ...
    if (ret != ESP_OK || bytes_read != 1) {
      ESP_LOGW(s_TAG, "  0x%02X: [READ ERROR: %s]", reg, esp_err_to_name(ret));
      values[i] = 0;
    }
  }
...
esp_err_t star_bus_debug_logger_print(const star_bus_transaction_logger_t* logger)
{
  ...
  ESP_LOGI(s_TAG, "=== Transaction Log (%zu entries) ===", logger->count);

  if (logger->count == 0) {
    ESP_LOGI(s_TAG, "  (empty)");
    return ESP_OK;
  }
...
esp_err_t star_bus_debug_stats_print(const star_bus_error_stats_t* stats)
{
  ...
  ESP_LOGI(s_TAG, "  Total transactions: %" PRIu32 "", stats->total_transactions);
  ESP_LOGI(s_TAG, "  Failed transactions: %" PRIu32 "", stats->failed_transactions);
  ...
  ESP_LOGI(s_TAG, "  Timeout errors: %" PRIu32 "", stats->timeout_errors);
  ESP_LOGI(s_TAG, "  NACK errors: %" PRIu32 "", stats->nack_errors);
  ESP_LOGI(s_TAG, "  Bus errors: %" PRIu32 "", stats->bus_errors);
  ESP_LOGI(s_TAG, "  Other errors: %" PRIu32 "", stats->other_errors);
...
esp_err_t star_bus_debug_test_bus(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(s_TAG, "=== Bus Test: %s ===", bus_name);

  if (!config->initialized) {
    ESP_LOGE(s_TAG, "  FAIL: Bus not initialized");
    return ESP_ERR_INVALID_STATE;
  }

  ESP_LOGI(s_TAG, "  PASS: Bus initialized");
  ...
  if (ret == ESP_OK) {
    ESP_LOGI(s_TAG, "  PASS: Bus scan completed (%zu device(s) found)", num_found);
  } else {
    ESP_LOGE(s_TAG, "  FAIL: Bus scan failed");
    return ret;
  }

  ESP_LOGI(s_TAG, "=== Bus Test PASSED ===");
  return ESP_OK;
}
...
esp_err_t star_bus_debug_test_device(const star_bus_manager_t* manager, const char* bus_name)
{
  ...
  ESP_LOGI(s_TAG, "=== Device Test: %s ===", bus_name);
  ...
  if (ret == ESP_OK) {
    ESP_LOGI(s_TAG, "  PASS: Device responds (read 0x%02X)", data);
  } else {
    ESP_LOGE(s_TAG, "  FAIL: Device does not respond: %s", esp_err_to_name(ret));
    return ret;
  }

  ESP_LOGI(s_TAG, "=== Device Test PASSED ===");
  return ESP_OK;
}
                                     ...
                                     uint8_t                   test_value)
{
  ...
  ESP_LOGI(s_TAG, "=== Register R/W Test: %s (reg=0x%02X) ===", bus_name, reg_addr);
  ...
  if (ret != ESP_OK || bytes_read != 1) {
    ESP_LOGE(s_TAG, "  FAIL: Could not read original value: %s", esp_err_to_name(ret));
    return ret;
  }

  ESP_LOGI(s_TAG, "  Original value: 0x%02X", original);
  ...
  if (ret != ESP_OK || bytes_written != 1) {
    ESP_LOGE(s_TAG, "  FAIL: Could not write test value: %s", esp_err_to_name(ret));
    return ret;
  }

  ESP_LOGI(s_TAG, "  Wrote test value: 0x%02X", test_value);
  ...
  if (ret != ESP_OK || bytes_read != 1) {
    ESP_LOGE(s_TAG, "  FAIL: Could not read back value: %s", esp_err_to_name(ret));
    return ret;
  }

  ESP_LOGI(s_TAG, "  Read back value: 0x%02X", readback);

  // Restore original value
  ret = star_bus_i2c_write(manager, bus_name, &original, 1, reg_addr, &bytes_written);

  if (ret != ESP_OK || bytes_written != 1) {
    ESP_LOGW(s_TAG, "  WARNING: Could not restore original value: %s", esp_err_to_name(ret));
  }

  // Verify
  if (readback == test_value) {
    ESP_LOGI(s_TAG, "  PASS: Register R/W verified");
    ESP_LOGI(s_TAG, "=== Register R/W Test PASSED ===");
    return ESP_OK;
  } else {
    ESP_LOGE(s_TAG, "  FAIL: Read back 0x%02X, expected 0x%02X", readback, test_value);
    return ESP_FAIL;
  }
}
