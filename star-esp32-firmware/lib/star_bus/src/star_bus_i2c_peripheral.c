...
/**
 * @brief Peripheral task - handles I2C peripheral communication
 */
static void internal_peripheral_task(void* param)
{
  ...
  while (state->task_running) {
    /* Read from I2C peripheral buffer */
    ...
    /* Check if controller is reading from us */
    /* Note: For ESP32 I2C peripheral, we pre-load data into the tx buffer
     * The driver will send this data when controller requests it */
    if (state->tx_data_length > 0) {
      /* Data is already loaded, just track stats when it gets sent */
      /* ESP32 I2C peripheral driver doesn't provide easy way to detect when
                                         ...
                                         const star_i2c_peripheral_config_t* config)
{
  ...
  /* Note: SDA and SCL pins must already be configured in controller mode setup */
  /* This function only changes the mode to peripheral */
  ...
  /* Install I2C driver in peripheral mode */
                                               ...
                                               size_t              length)
{
  ...
  /* Write to I2C peripheral tx buffer */
  int32_t written =
    i2c_slave_write_buffer(state->port, data, length, pdMS_TO_TICKS(state->config.timeout_ms));

  if (written < 0) {
    ESP_LOGE(s_TAG, "Failed to write to peripheral tx buffer");
    return ESP_FAIL;
  }
