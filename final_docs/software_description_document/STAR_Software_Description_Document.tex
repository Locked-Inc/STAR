...
\node[sddbox] (ui) {Grafana cockpit\\(panel-29 + Pi5 API)};
...
\item \textbf{star-proto} owns the language-neutral API contract and
      emits Go, C (nanopb), and C++ bindings.
...
\item \textbf{star-gateway} owns the five-layer protocol stack and
      bridges the Grafana cockpit (via the Pi5 cockpit-API) to both
      ROS2 and the RX72N.
\item \textbf{star-ros2} owns SLAM, sensor fusion, Nav2 planning,
      lifecycle-managed safety monitoring, and action servers.
\item \textbf{Grafana cockpit + Pi5 cockpit-API} (\texttt{infra/pi5/}
      and \texttt{monitoring/grafana-star-dashboard.json}) owns all
      user-facing operator workflows: teleoperation (panel-29),
      autonomy mode switching, telemetry plotting, and
...
exceptions) and exception-based in C++ ROS2 nodes; \textbf{logging}
uses the native facility of each runtime
(\texttt{printf}-like \texttt{rx\_log\_*} on RX72N, structured logs in
Go, \texttt{RCLCPP\_*} in C++, Grafana / Loki for the cockpit);
...
\texttt{buf.build/grpc/go}; C bindings are generated by
...
\subsection{Grafana Cockpit (Operator Dashboard)}
\textbf{Purpose:} Provide a browser-based operator dashboard for
teleoperation, autonomy mode selection, live telemetry plotting, and
compliance-report viewing.

\textbf{Stack:} Grafana dashboard JSON
(\texttt{monitoring/grafana-star-dashboard.json}) backed by the Pi5
\texttt{star\_cockpit\_api} Flask service
(\texttt{infra/pi5/opt/star\_cockpit\_api/server.py}) at port
\texttt{:9102}, served behind Caddy. Grafana panel-29 hosts the
teleoperation / map / compliance UI; the dashboard reads ROS2 state
through the cockpit-API and time-series metrics through Prometheus.

\textbf{Panel inventory:} Live map view, command velocity controls,
motor plots, IMU orientation display, LiDAR scan overlay, alert log,
autonomy mode switch, emergency-stop panel, SLAM control, and the
compliance-report viewer -- all defined declaratively in the
dashboard JSON.

\textbf{Reconnection:} HTTP polling against the cockpit-API and
Prometheus; Grafana handles transient backend errors and degrades
gracefully to ``no data'' indicators when the underlying datasource
is unreachable.
...
Operator UI         & Grafana cockpit       & Declarative dashboard JSON, no custom front-end maintenance \\
Cockpit time-series & Prometheus            & Pull-based scraping; native Grafana datasource \\
