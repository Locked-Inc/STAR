...
status \emph{[ARCHITECTED]} target for production. The transport
...
should be read as \emph{[ARCHITECTED]} unless the surrounding text
...
\texttt{star\_simple\_bridge} (\emph{[IMPLEMENTED]} -- the deployed
USB-CDC ASCII bridge to the RX72N at 115200~baud; serves
OccupancyGrid PNG and Prometheus metrics on \texttt{:9101}),
\texttt{star\_spi\_bridge} (\emph{[ARCHITECTED]} -- the production
...
\item \textbf{Ramp width} (\texttt{ramp\_width\_node.py}),
      ...
      (\texttt{compliance\_monitor\_node.py}) -- [ARCHITECTED stubs].
