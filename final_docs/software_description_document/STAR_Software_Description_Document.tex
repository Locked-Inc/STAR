...
Industry surveys estimate that approximately 73 percent of public-facing
facilities fail at least one ADA Title III requirement, and the Seyfarth
Shaw annual ADA Title III report shows federal lawsuit filings growing
steadily from 2018 through 2024. Manual audits by certified inspectors
remain expensive and slow, creating a structural gap between the volume
of non-compliant spaces and the rate at which they can be identified and
remediated. STAR exists to close that gap by replacing discretionary
manual measurement with deterministic, sensor-grounded, repeatable
...
provisioning scripts, and the ADA compliance-audit engine. The STM32
peripheral and BeagleBone Black bring-up targets are intentionally
outside the scope of this SDD.
...
star-rx72n-firmware           & Cesar Magana & Brighton Sikarskie \\
...
infra/ + scripts/ (cockpit)   & Cesar Magana & Brighton Sikarskie \\
compliance-engine (ADA)       & Cesar Magana & Brighton Sikarskie \\
\bottomrule
\end{tabularx}
\end{table}
...
\item STM32 peripheral firmware (covered in a separate hardware
      deliverable).
\item BeagleBone Black bring-up and secondary compute work (retired
      target; retained only as a historical reference).
...
ADA Title III Section 405.2 requires that newly constructed or altered
ramps not exceed a 1:12 rise-to-run slope. In practice, inspectors
audit this and related geometric constraints by hand, using
inclinometers, tape measures, and photography. Manual auditing is
slow, subjective, and error-prone. Filed lawsuits against
non-compliant facilities have grown every year from 2018 through 2024
(Seyfarth Shaw ADA Title III report, 2024), yet the rate of formal
audit completions has not kept pace. STAR addresses this gap by
combining an autonomous mobile platform with a deterministic software
pipeline that measures ramp slope, path width, trip-hazard height, and
related ADA geometry from sensor data with known tolerances.
...
\textbf{Packages:} \texttt{star\_bringup} (launch orchestration),
\texttt{star\_spi\_bridge} (ROS2-side SPI wrapper),
...
\texttt{star\_supervisor} (top-level state coordinator), and
\texttt{sllidar\_ros2} (vendor driver for the RPLiDAR C1).
...
\textbf{Status-label legend (applies throughout this subsection):}
...
\item \textbf{Ramp slope} (\texttt{ramp\_slope\_node.py}) --
      [IMPLEMENTED]. RANSAC plane fit to LiDAR points in a ramp
      region of interest, cross-validated against RX72N IMU pitch.
\item \textbf{Trip hazard} (\texttt{trip\_hazard\_node.py}) --
      [STRETCH stub]. Detects vertical discontinuities above the ADA
      threshold (one quarter inch).
\item \textbf{Path width} (\texttt{path\_width\_node.py}) --
      [STRETCH stub]. Occupancy-grid cross-section width along the
      planned path.
\item \textbf{Ramp width, ramp landing, door clear width, door
      threshold} -- [ARCHITECTED]. Message schemas, node skeletons,
      and traceability-matrix rows are in place; the internal
      algorithm bodies are not yet implemented.
\end{itemize}
...
peripheral (slave) mode with DMA-backed ping-pong buffers.
...
\item Americans with Disabilities Act, Title III, Section 405.2
      (Running Slope of Ramps).
\item Seyfarth Shaw LLP, ADA Title III Report, 2024.
\item NASA/JPL Institutional Coding Standard: ``The Power of 10:
      Rules for Developing Safety-Critical Code.''
\item Boston Dynamics Proto3 Style Guide, public release.
\item Open Source Robotics Foundation, Nav2 documentation,
      \url{https://docs.nav2.org/}.
\item \texttt{slam\_toolbox} project, S. Macenski et al.
\item nanopb, Petteri Aimonen,
      \url{https://github.com/nanopb/nanopb}.
\item Azure RTOS ThreadX, Microsoft.
\item ROS2 Jazzy Jalisco release documentation.
\item S. Macenski and I. Jambrecic, \textit{SLAM Toolbox: SLAM for
      the dynamic world}.
\end{enumerate}
...
\midrule
405.2 Ramp slope          & LiDAR + IMU  & RANSAC plane + IMU cross-check & ramp\_slope\_node     & [IMPLEMENTED] \\
303 Trip hazard           & LiDAR        & Vertical discontinuity detector & trip\_hazard\_node    & [STRETCH] \\
403.5 Path width          & LiDAR map    & Cross-section width along path  & path\_width\_node     & [STRETCH] \\
405.5 Ramp width          & LiDAR        & Lateral extent of ramp plane    & ramp\_width\_node     & [ARCHITECTED] \\
405.7 Ramp landings       & LiDAR        & Adjacent plane detection        & ramp\_landing\_node   & [ARCHITECTED] \\
404.2.3 Door clear width  & LiDAR        & Doorway passable-width scan     & door\_width\_node     & [ARCHITECTED] \\
404.2.5 Door threshold    & LiDAR        & Threshold height measurement    & door\_threshold\_node & [ARCHITECTED] \\
\bottomrule
\end{tabularx}
\end{table}
