...
\usepackage[colorlinks=true,
            ...
            pdfkeywords={STAR, testing, Unity, pytest, CI, coverage,
                         IEEE 29119}]{hyperref}
...
\subsection{Scope}
The report covers seven software subsystems: the RX72N real-time
firmware, the Go gateway, the ROS2 Jazzy autonomy stack, the
Python compliance engine, the Protocol Buffers schema repository, the
virtual-RX72N simulator, and the CI/CD automation that drives the
whole pipeline. The Grafana cockpit (panel-29 plus the Pi5
\texttt{star\_cockpit\_api}) provides the operator interface and is
exercised end-to-end at HIL time but is not unit-tested in isolation.
Physical PCB and mechanical verification are out of scope for this
report.
...
\item \textbf{Unit tests} run on host with native compilers and
      ...
      for the gateway, \texttt{pytest} for compliance, C Unity for
      nanopb).
...
Two parallel test harnesses validate that Go and nanopb encode
identical bytes for the same canonical fixtures.
...
\subsection{Results}
Both serialization harnesses pass at HEAD. Buf lint, format, and
build are green; no open breaking-change violations.

%======================================================================
% 8. Operator UI (Grafana cockpit)
%======================================================================
\section{Operator UI Tests (Grafana Cockpit)}
\label{sec:ui}

\subsection{Scope}
The production operator interface is a Grafana cockpit
(\texttt{monitoring/grafana-star-dashboard.json}) backed by the Pi5
\texttt{star\_cockpit\_api}
(\texttt{infra/pi5/opt/star\_cockpit\_api/server.py}). It is
exercised end-to-end at HIL time -- by walking the operator through
panel-29 for teleoperation, autonomy mode switching, SLAM control,
and compliance-report viewing -- rather than via a frontend unit-test
harness. There is no legacy React UI in the tree any more; the only
remaining UI surface is the dashboard JSON and the Flask cockpit-API
service.

\subsection{Status}
The Grafana cockpit is exercised end-to-end at HIL time. There are
no frontend unit suites in the repository -- by design, since the
dashboard is declarative JSON. Tests for the underlying surface
exist at the protocol layer: gateway tests and compliance-engine
tests cover every endpoint the cockpit-API exposes.
...
Operator UI (Grafana)            & end-to-end   & \pass{} & Exercised at HIL time \\
...
The operator interface is the Grafana cockpit (dashboard JSON plus
Pi5 cockpit-API). It is exercised end-to-end at HIL time and is
covered indirectly by the gateway and compliance-engine test suites
that exercise every endpoint the cockpit-API invokes. This is
recorded in Section~\ref{sec:ui}.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{UI coverage} -- the Grafana cockpit is exercised
      end-to-end at HIL time rather than via a frontend unit-test
      harness. Mitigation: the cockpit's underlying surface (the
      Pi5 cockpit-API and the gateway it invokes) is fully covered
      by the Go and Python test suites.
