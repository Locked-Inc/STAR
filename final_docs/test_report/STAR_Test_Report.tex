...
informed by IEEE 29119-3 (Test Documentation) and IEEE 829 legacy
practice, adapted to the multi-language, multi-target STAR codebase.
...
Architecture and design details are described in the STAR Software
Description Document (SDD), and operational procedures in the STAR
System and Software User Manual (SSUM). Where this report refers to
a design decision without elaboration, the SDD is authoritative.
...
\subsection{Results}
Table~\ref{tab:fw-results} summarizes the most recent CI-captured
JUnit artifact at
\texttt{e2-studio-star-rx72n-firmware/tests/build/test-results.xml}.

\begin{table}[H]
\centering
\caption{Firmware unit test results (JUnit XML, 2026-03-01).}
...
\subsection{Scope and Inventory}
Forty-six \texttt{*\_test.go} files across twenty-one packages exercise
...
\texttt{internal/frame/decoder.go} measure at 100.0 percent; the
\texttt{ws/hub} hot-loop paths (\texttt{shouldThrottle},
\texttt{stampAndFanOut}) are at 100.0 percent and 77.8 percent
respectively; the lowest-coverage routine flagged is
\texttt{ws/hub.route} at 50.0 percent (pending a refactor to expose
its private branches to unit tests).
...
\subsection{Scope}
Fifteen test files split between C++ (ament\_cmake\_gtest) and Python
(ament\_python with \texttt{launch\_test}). Tests are invoked by
\texttt{colcon test --return-code-on-test-failure} in
...
The gap between this single suite and the seventeen-panel inventory
of the original React UI is acknowledged explicitly: the UI is
[STRETCH] as delivered, since the Grafana and Lichtblick dashboards
...
expanded to match its original seventeen-panel scope; instead, the
...
\item \textbf{UI coverage} -- single Vitest suite against a
      seventeen-panel inventory. Mitigation: the Grafana/Lichtblick
      dashboards are exercised at the API layer by gateway tests;
      the React UI is [STRETCH] rather than required.
...
46 firmware unit tests at 100 percent library coverage, 46 Go test
files at 74 percent gateway coverage, fifteen ROS2 tests including
four safety-critical launch tests, twelve compliance-engine pytest
modules, nine Protobuf serialization tests spanning three languages,
eight on-bench hardware bring-up programs, and fourteen CI
workflows that enforce the gates described in
...
suite, which is acknowledged as [STRETCH] because the dashboarding
...
\item \texttt{e2-studio-star-rx72n-firmware/tests/build/test-results.xml}
      -- firmware JUnit XML, 46/46 tests, 0 failures.
