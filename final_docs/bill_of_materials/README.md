...
PCB (6-layer, Renesas RX72N, motor drivers, power distribution,
...
These per-subcircuit CSVs are sourced manually from
`Schematic/BOM/` and use a richer 20-column schema (Manufacturer
Name, DigiKey Part Number, Mouser Link, etc.) suited for sourcing.
They are not regenerated by `kicad-cli`; only the primary
`STAR_MCU_BOM.csv` (8 columns) is regenerable from the schematic
source via the command in the regeneration section below.
...
Primary schematic: `Schematic/STAR_MCU.kicad_sch`.
Primary PCB: `Schematic/STAR_MCU.kicad_pcb`.
