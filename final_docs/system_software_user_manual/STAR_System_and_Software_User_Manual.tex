...
each subsystem or feature is on the delivered system:
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
...
\item ADA 2010 Standards, Section 405.2 (Ramp Slope).
\end{itemize}
...
Caddy (Pi5-side)        & Pi5 \texttt{:443}
    & Local-CA TLS front for \texttt{star.local}. [IMPLEMENTED] \\
...
\texttt{flash-rx72n.sh}  & Workstation
    & One-shot firmware flashing helper. [IMPLEMENTED] \\
\bottomrule
\end{tabularx}
\end{table}
...
robot is moving. The ramp-slope check is [IMPLEMENTED]; other checks
are [STRETCH] or [ARCHITECTED] per the SDD traceability matrix. The
report generator in
...
\end{itemize}
Plots beside the 3D panel show IMU acceleration and angular velocity
on all three axes.
...
\item Compliance coverage: only the ramp-slope check is
      [IMPLEMENTED]; path width and trip hazard are [STRETCH], and
      ramp width, ramp landings, door clear width, and door
      threshold are [ARCHITECTED] per the SDD traceability matrix.
\item \texttt{star-simple} (a planned lightweight bring-up tree on
      the field Pi5) is [ARCHITECTED]: some of it is present on the
