...
every internal document, configuration, and script referenced
below, lives at
\url{https://github.com/Locked-Inc/STAR}. Each entry below links to
its current location on GitHub's main branch; click through to read
the source.

\textbf{In-repo references} (open-source, follow each link):
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
...
\item End-to-end operations stack topology (Pi5, Tailscale, k3s,
      Cloudflare):
      \href{https://github.com/Locked-Inc/STAR/blob/main/docs/ARCHITECTURE.md}{\texttt{docs/ARCHITECTURE.md}}.
\item Pi5-side system files, Caddy configuration, systemd units,
      fresh-Pi5 install procedure:
      \href{https://github.com/Locked-Inc/STAR/blob/main/infra/README.md}{\texttt{infra/README.md}}.
\item ROS2 Jazzy workstation and Pi5 provisioning:
      \href{https://github.com/Locked-Inc/STAR/blob/main/docs/PI_DEPLOYMENT.md}{\texttt{docs/PI\_DEPLOYMENT.md}}.
\item RX72N firmware flashing helper:
      \href{https://github.com/Locked-Inc/STAR/blob/main/scripts/flash-rx72n.sh}{\texttt{scripts/flash-rx72n.sh}}.
\item Grafana dashboard used by the operator cockpit:
      \href{https://github.com/Locked-Inc/STAR/blob/main/monitoring/grafana-star-dashboard.json}{\texttt{monitoring/grafana-star-dashboard.json}}.
\item MVP ROS2 bridge that serves Prometheus metrics and map images:
      \href{https://github.com/Locked-Inc/STAR/tree/main/star-ros2/src/star_simple_bridge}{\texttt{star-ros2/src/star\_simple\_bridge/}}.
\item Default Foxglove / Lichtblick layout:
      \href{https://github.com/Locked-Inc/STAR/blob/main/star-ros2/src/star_bringup/foxglove/star-default.json}{\texttt{star-ros2/src/star\_bringup/foxglove/star-default.json}}.
\item One-command bring-up helper:
      \href{https://github.com/Locked-Inc/STAR/blob/main/scripts/slam-mvp.sh}{\texttt{scripts/slam-mvp.sh}}.
\item Local Lichtblick web bundle setup:
      \href{https://github.com/Locked-Inc/STAR/blob/main/scripts/setup-lichtblick-web.sh}{\texttt{scripts/setup-lichtblick-web.sh}}.
\end{itemize}
...
\item Install ROS 2 Jazzy from the Open Robotics apt repository
      (\texttt{packages.ros.org}) via the \texttt{ros2-apt-source}
      package, and source
...
The Grafana dashboard embeds the STAR map served by
\texttt{star\_simple\_bridge} directly over the tailnet at
\texttt{http://100.64.0.6:9101/map.png} (plain HTTP, no certificate
required). For other tabs that hit the Pi5 Caddy front
(\texttt{https://star.local/}), the operator's browser must trust
the Pi5 Caddy's local CA -- see \texttt{docs/ARCHITECTURE.md} for
the one-time browser trust-store step.
...
on a developer workstation. The five subcommands:
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
\item \texttt{slam-mvp.sh start [--mcu /dev/star-mcu]
      [--lidar /dev/star-lidar]} -- launches each ROS2 node in the
      SLAM MVP set as a separate background process with its own
      log file under \texttt{/tmp/slam-mvp-logs/}. PIDs land in
      \texttt{/tmp/slam-mvp.pids}.
\item \texttt{slam-mvp.sh start-nav} -- same as \texttt{start} plus
      Nav2 (planner, controller, behavior server) on top, for
      AUTONOMY-mode bring-up.
...
The Pi5 Caddy front (\texttt{https://star.local/}) uses a local CA
to issue its certificate, so cross-tab requests like
\texttt{curl https://star.local/api/state} require the operator's
laptop to trust the Pi5's local-CA root. Install that CA on the
laptop one time per \texttt{docs/ARCHITECTURE.md}. The committed
Grafana dashboard panel loads its map image from
\texttt{http://100.64.0.6:9101/map.png} on the tailnet (plain HTTP,
no certificate involved), so the local CA is not required for the
map panel itself; it is required for any browser interaction with
\texttt{star.local}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\linewidth]{images/thompson-hall-map.png}
\caption{A finished SLAM occupancy grid produced by STAR in
Thompson Hall on the Texas A\&M campus. White cells are free
space, black cells are detected obstacles (walls, doorways,
furniture), and grey is unobserved. This is the same image format
the Grafana cockpit map panel and the
\texttt{http://100.64.0.6:9101/map.png} endpoint serve; download
\texttt{.pgm} and \texttt{.yaml} from the cockpit (see
Figure~\ref{fig:grafana-cockpit}) to feed the map back into
Nav2.}
\label{fig:thompson-hall-map}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{images/grafana-cockpit.png}
\caption{The STAR Grafana cockpit. The CONTROL row holds the
E-STOP toggle, the MANUAL/AUTONOMY toggle, and the SPEED slider
(percent of \texttt{MAX\_WHEEL\_MPS}). The TURN field publishes a
test \texttt{angular.z} pulse in \unit{\radian\per\second}. Below
that, MAP gives \texttt{Download .pgm} / \texttt{Download .yaml}
exports plus \texttt{STOP MAPPING} and \texttt{CLEAR MAP} buttons,
the live SLAM map panel, and the eight-direction WASD-equivalent
keypad. ``ok 7:20:26 AM'' is the heartbeat indicator from
\texttt{star\_cockpit\_api}.}
\label{fig:grafana-cockpit}
\end{figure}

The cockpit you see in Figure~\ref{fig:grafana-cockpit} has two
parts that share the same Grafana page:
\begin{enumerate}[leftmargin=1.4em,itemsep=2pt]
\item \textbf{Read-only telemetry from the committed JSON.} The
      file \texttt{monitoring/grafana-star-dashboard.json} defines
      Grafana panels that are pure visualization: row groups for
      SLAM map, motors, safety, sensors, and SLAM diagnostics.
      These panels read Prometheus metrics scraped from the Pi5
      and render gauges, time series, and the embedded map image
      from \texttt{http://100.64.0.6:9101/map.png} (the
      \texttt{star\_simple\_bridge} HTTP server on the tailnet).
      The dashboard JSON contains no buttons or forms; everything
      defined inside it is read-only.
\item \textbf{Cockpit control widgets at the top of the page.}
      The CONTROL row -- E-STOP toggle, MANUAL/AUTONOMY toggle,
      SPEED slider with Apply, TURN field, MAP \texttt{Download
      .pgm} / \texttt{Download .yaml} / \texttt{STOP MAPPING} /
      \texttt{CLEAR MAP} buttons, and the eight-direction WASD
      keypad -- is an HTML control surface that POSTs to
      \texttt{star\_cockpit\_api} on \texttt{:9102} (see
      Table~\ref{tab:cockpit}). It is layered on top of the
      Grafana dashboard at deployment time and is not part of the
      committed JSON.
\end{enumerate}

The dashboard JSON's own panels include the per-wheel motor
gauges keyed on \texttt{star\_motor\_velocity\_mps\{wheel=fl|fr|bl|br\}}
and the embedded map image at
\texttt{http://100.64.0.6:9101/map.png}. The dashboard sets its
own \texttt{refresh} field to \qty{1}{\second} -- there is no
\texttt{GF\_DASHBOARDS\_MIN\_REFRESH\_INTERVAL} override required.

\subsection{Lichtblick Layout}
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{images/lichtblick-teleop.png}
\caption{Lichtblick layout in operator use. The center 3D panel
shows the live SLAM map (occupancy grid) with the
\texttt{laser\_frame} and \texttt{odom} TF axes overlaid. The
right-rail Teleop panel publishes \texttt{geometry\_msgs/Twist}
on \texttt{/cmd\_vel} at \qty{10}{\hertz}; the down-arrow is
highlighted because it is being held. The left sidebar exposes
the per-button \texttt{linear-x} / \texttt{angular-z} bindings
the layout uses.}
\label{fig:lichtblick-teleop}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth]{images/foxglove-studio.png}
\caption{Foxglove Studio desktop app reading the same layout JSON
the web Lichtblick uses. Center: live 3D scene with the
\texttt{/scan} rays around \texttt{laser\_frame} on top of the
SLAM occupancy grid. Bottom: time-series plots of
\texttt{/imu/data.linear\_acceleration} and
\texttt{angular\_velocity} on all three axes. Right: the
Diagnostics summary and the foxglove-bridge log stream.}
\label{fig:foxglove-studio}
\end{figure}
...
9100 & star\_simple\_bridge      & Prometheus \texttt{/metrics} (\texttt{PROMETHEUS\_PORT}) \\
9101 & star\_simple\_bridge      & \texttt{/map.png}, \texttt{/map.pgm}, \texttt{/map.yaml}, \texttt{POST /map/reset} (\texttt{MAP\_HTTP\_PORT}) \\
...
\begin{table}[H]
\centering
\caption{Operator-relevant ROS2 topics. Compliance check topics
(e.g. \texttt{/compliance/ramp\_slope}, \texttt{/compliance/report})
are not yet wired up on the bridge -- the engine is run as an
offline post-processing step today; see Section~\ref{sec:limits}.}
\label{tab:topics}
\small
\begin{tabularx}{\linewidth}{l l X}
\toprule
...
\texttt{enable\_auto\_estop}        & true  & Primary enable \\
...
\texttt{star\_simple\_bridge} has not produced a map yet (SLAM not
started, or no scans accumulated), or the Pi5 is unreachable on the
tailnet. Confirm
\texttt{curl http://100.64.0.6:9101/map.png} from any machine on
the tailnet returns an image. If it returns 503, start SLAM through
the cockpit; if it times out, check that
\texttt{star-slam-mvp.service} is running on the Pi5.
